...
class InProcessWorker:
    ...
    def cancel_error(self, code: str, *, outcome_known: bool) -> None:
        self.failure = WorkerCancellationError(
            code=code, outcome_known=outcome_known
        )

    async def start_task(self, request):
        raise AssertionError("exit integration never starts a Worker")

    def stream_events(self, handle):
        raise AssertionError("exit integration never streams a Worker")

    async def respond_permission(self, handle, **kwargs):
        raise AssertionError("exit integration never answers permission")
    ...
    async def collect_result(self, handle):
        raise AssertionError("exit integration never collects a result")
...
class ExitHarness:
    def __init__(self, root: Path) -> None:
        ...
        self.coordinator = AsyncExitCoordinator(
            exit_service=self.exit_service, store=self.store,
            clock=FrozenClock(NOW), runtime=self.runtime,
            lifecycle=self.lifecycle, session_id="ses_1",
        )
    ...
    def attempt_state(self):
        return self.store.load_aggregate("attempts", "att_1")["state"]

    def session_state(self):
        return self.store.load_aggregate("product_sessions", "ses_1")["state"]
    ...
    def event_kinds(self):
        return tuple(row[0] for row in self.store._require_writer().execute(
...
async def test_cancel_failure_closes_replay_without_false_interruption(
    ...
    try:
        ...
        assert first == second
        assert first.diagnostic.code == worker_code
...
@async_test
async def test_terminal_win_then_next_exit_pauses_without_second_cancel(tmp_path):
    harness = ExitHarness(tmp_path)
    try:
        harness.bind()
        ...
        def terminal_win():
            harness.store._require_writer().execute(
                "UPDATE attempts SET state='completed',reason=NULL,"
                "result_summary='done',updated_at=? WHERE attempt_id='att_1'",
                (NOW.isoformat(),),
            )
            harness.runtime.release("att_1", harness.handle)

        harness.worker.on_cancel = terminal_win
        result = await harness.coordinator.confirm(
            harness.request.request_id, harness.request.attempt_hash
        )
        assert result.diagnostic.code == "exit_authority_changed_after_cancel"
        assert harness.attempt_state() == "completed"
        paused = await harness.coordinator.request_exit()
...
async def test_cross_session_coordinator_closes_before_io_or_write(tmp_path):
    ...
    try:
        ...
        coordinator = AsyncExitCoordinator(
            exit_service=harness.exit_service, store=harness.store,
            clock=FrozenClock(NOW), runtime=harness.runtime,
            lifecycle=harness.lifecycle, session_id="ses_other",
        )
        before = harness.database_facts()
