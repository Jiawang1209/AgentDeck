...
from agentdeck.providers.plan_schema import LEADER_PLAN_MAX_STEPS
...
def cli_plan_stdout_blank_goal() -> str:
    return json.dumps(
        {
            "goal": "",
            "summary": "provider returned an unusable goal",
            "steps": [
                {
                    "step": 1,
                    "agent_id": "planner",
...
def cli_plan_stdout_non_string_step_agent_id() -> str:
    return json.dumps(
        {
            "goal": "Malformed provider plan",
            "summary": "provider returned an unusable agent id",
            "steps": [
                {
                    "step": 1,
                    "agent_id": 123,
...
def cli_plan_stdout_non_positive_step_number() -> str:
    return json.dumps(
        {
            "goal": "Malformed provider plan",
            "summary": "provider returned an unusable step number",
            "steps": [
                {
                    "step": 0,
                    "agent_id": "planner",
...
def cli_plan_stdout_non_integer_step_number() -> str:
    return json.dumps(
        {
            "goal": "Malformed provider plan",
            "summary": "provider returned an unusable step number",
...
def cli_plan_stdout_non_contiguous_step_numbers() -> str:
    return json.dumps(
        {
            "goal": "Malformed provider plan",
            "summary": "provider returned non-contiguous step numbers",
            "steps": [
                {
                    "step": 1,
                    "agent_id": "planner",
...
def test_deepseek_provider_uses_deepseek_env_and_openai_compatible_plan_shape(tmp_path, monkeypatch) -> None:
    ...
    assert (
        f"Plan between 1 and {LEADER_PLAN_MAX_STEPS} steps; use only as many steps as the task actually needs."
...
def test_openai_compatible_provider_rejects_non_string_step_display_fields(
    tmp_path, monkeypatch
) -> None:
    root = tmp_path / "repo"
    root.mkdir()
    ...
    class NonStringStepFieldResponse:
        def __enter__(self) -> "NonStringStepFieldResponse":
            return self
            ...
            return json.dumps(
                {
                    "choices": [
                        {
                            "message": {
                                "content": cli_plan_stdout_non_string_step_agent_id(),
                            }
                        }
                    ]
                }
    ...
    monkeypatch.setattr(
        "agentdeck.providers.openai_compatible.request.urlopen",
        lambda _request, timeout: NonStringStepFieldResponse(),
    )

    provider = OpenAICompatibleProvider()

    try:
        provider.plan(LeaderPlanRequest(task="拒绝非字符串 step 字段 API plan", config=config))
    except RuntimeError as exc:
...
def test_openai_compatible_provider_rejects_non_integer_step_numbers(
    tmp_path, monkeypatch
) -> None:
    root = tmp_path / "repo"
    root.mkdir()
    ...
    class NonIntegerStepResponse:
        def __enter__(self) -> "NonIntegerStepResponse":
            return self
            ...
            return json.dumps(
                {
                    "choices": [
                        {
                            "message": {
                                "content": cli_plan_stdout_non_integer_step_number(),
                            }
                        }
                    ]
                }
    ...
    monkeypatch.setattr(
        "agentdeck.providers.openai_compatible.request.urlopen",
        lambda _request, timeout: NonIntegerStepResponse(),
    )

    provider = OpenAICompatibleProvider()

    try:
        provider.plan(LeaderPlanRequest(task="拒绝不可排序 API plan", config=config))
    except RuntimeError as exc:
...
def test_openai_compatible_provider_rejects_non_contiguous_step_numbers(
    tmp_path, monkeypatch
) -> None:
    root = tmp_path / "repo"
    root.mkdir()
    ...
    class NonContiguousStepResponse:
        def __enter__(self) -> "NonContiguousStepResponse":
            return self
            ...
            return json.dumps(
                {
                    "choices": [
                        {
                            "message": {
                                "content": cli_plan_stdout_non_contiguous_step_numbers(),
                            }
                        }
                    ]
                }
    ...
    monkeypatch.setattr(
        "agentdeck.providers.openai_compatible.request.urlopen",
        lambda _request, timeout: NonContiguousStepResponse(),
    )

    provider = OpenAICompatibleProvider()

    try:
        provider.plan(LeaderPlanRequest(task="拒绝跳号 API plan", config=config))
    except RuntimeError as exc:
