...
def _validate_fact_key_value(key: str, value: object) -> None:
    ...
    try:
        ...
        raise StoreSerializationError("canonical object key must be strict UTF-8") from error
    if len(encoded) > 65_536:
        raise StoreSerializationError("canonical object key is too large")
    ...
    if collapsed in _DIGEST_METADATA:
        if (
            ...
            raise StoreSerializationError("provenance digest must be 64 lowercase hex")
        return
    if collapsed == _SOURCE_METADATA:
        if not _valid_source_label(value):
            raise StoreSerializationError("credential source name is invalid")
        return
    if collapsed in _SAFE_USAGE_METADATA:
        if type(value) is not int or value < 0:
            raise StoreSerializationError("token count must be a nonnegative integer")
        return
    ...
    if credential or raw_fact:
        raise StoreSerializationError("raw secret or credential facts are prohibited")
