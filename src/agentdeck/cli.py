...
def _leader_chat_intent_card(payload: dict[str, object]) -> dict[str, object]:
    ...
    if embedded_card == "release_preview_card" and payload.get("control_registry_card") is not None:
        secondary_embedded_cards.append("control_registry_card")
    if embedded_card == "role_topology_card" and payload.get("control_registry_card") is not None:
        secondary_embedded_cards.append("control_registry_card")
    ...
    if embedded_card == "skill_context_card" and payload.get("control_registry_card") is not None:
        secondary_embedded_cards.append("control_registry_card")
    if embedded_card == "memory_context_card" and payload.get("control_registry_card") is not None:
        secondary_embedded_cards.append("control_registry_card")
    ...
    if embedded_card == "learning_review_card" and payload.get("control_registry_card") is not None:
        secondary_embedded_cards.append("control_registry_card")
    if embedded_card == "skill_suggestions_card" and payload.get("control_registry_card") is not None:
        secondary_embedded_cards.append("control_registry_card")
    if embedded_card == "memory_suggestions_card" and payload.get("control_registry_card") is not None:
        secondary_embedded_cards.append("control_registry_card")
...
def _leader_chat_intent_inspect_command(embedded_card: object, payload: dict[str, object]) -> str | None:
    ...
    if embedded_card == "release_preview_card":
        return "agentdeck workbench"
    if embedded_card == "role_topology_card":
        return "agentdeck workbench"
...
async def _load_acp_session(
    config: ProjectConfig, agent: AgentSpec, store: StateStore, session: dict[str, Any]
) -> dict[str, Any]:
    sink = _AcpRunLedgerSink(store)
    client = AgentDeckAcpClient(sink=sink, decide=lambda *_: PermissionDecision.cancelled("load_replay"))
    transport = AcpTransport(agent.transport_command, project_root(), client)
    turn: dict[str, Any] | None = None
    try:
        initialized = await transport.initialize()
        if not initialized.load_session:
            raise AcpTransportError("ACP Agent does not support loadSession")
        store.record_protocol_transition("session", session["session_id"], "disconnected", "reconnecting", "session_load_started", {})
        turn = store.record_protocol_turn(session["session_id"], new_id("msg"), "load_replay")
        store.record_protocol_transition("turn", turn["turn_id"], "created", "submitted", "session_load_submitted", {})
        sink.activate(session["native_session_id"], session["session_id"], turn["turn_id"])
        await transport.load_session(session["native_session_id"])
        store.record_transport_update(session["session_id"], turn["turn_id"], sink.sequence, "completion", {"stop_reason": "loaded"})
        sink.sequence += 1
...
async def _resume_acp_session(
    config: ProjectConfig, agent: AgentSpec, store: StateStore,
    session: dict[str, Any], prompt: str,
) -> dict[str, Any]:
    sink = _AcpRunLedgerSink(store)
    client = AgentDeckAcpClient(
        sink=sink,
        decide=lambda pending, options: foreground_permission_decider(pending, options, stdin=sys.stdin, stderr=sys.stderr),
    )
    transport = AcpTransport(agent.transport_command, project_root(), client)
    turn: dict[str, Any] | None = None
    cancellation_requested = False
    ...
    try:
        ...
        if not initialized.resume_session:
            raise AcpTransportError("ACP Agent does not support sessionCapabilities.resume")
...
def _resolve_acp_session(config: ProjectConfig, store: StateStore, session_id: str) -> tuple[dict[str, Any], AgentSpec]:
    state = store.validated_protocol_state()
    matches = [item for item in state["agent_sessions"] if item["session_id"] == session_id]
    if len(matches) != 1:
...
def _acp_reconnect_command(args: argparse.Namespace, *, resume: bool) -> int:
    if not args.confirm:
        print("--confirm is required to start an ACP Agent", file=sys.stderr)
        return 1
    if resume and (type(args.prompt) is not str or not args.prompt.strip()):
        print("--prompt must be non-empty", file=sys.stderr)
        return 1
    try:
        config = load_config(project_root())
        store = StateStore.open_existing(project_root())
        session, agent = _resolve_acp_session(config, store, args.session_id)
    ...
    if blockers:
        ...
        return 1
    try:
        payload = asyncio.run(_resume_acp_session(config, agent, store, session, args.prompt) if resume else _load_acp_session(config, agent, store, session))
    except (AcpTransportError, ValueError, KeyError) as error:
        print(str(error), file=sys.stderr)
        return 1
    validation = validate_acp_runtime_contract(payload)
...
def protocol_acp_load_command(args: argparse.Namespace) -> int:
    return _acp_reconnect_command(args, resume=False)


def protocol_acp_resume_command(args: argparse.Namespace) -> int:
    return _acp_reconnect_command(args, resume=True)
...
def _run_progress_payload(store: StateStore, plan_id: str) -> dict[str, object]:
    ...
    return {
        ...
        "verdict_summary": review.get("verdict_summary"),
        "review": review,
        "approval_card": approval_card,
        "next_command": next_command,
...
def _leader_summary_payload(store: StateStore, plan_id: str) -> dict[str, object]:
    ...
    return {
        ...
        "artifact_count": artifact_count,
        "verdict_summary": store.plan_verdict_summary(plan_id),
...
def _chat_wants_role_topology(message: str) -> bool:
    normalized = message.strip().lower()
    compact = normalized.replace(" ", "").replace("-", "")
    return normalized in {
        "role topology",
...
def _leader_chat_explanation(
    ...
    *,
    ...
    if mode == "plan_board":
        plan_board_card = result if isinstance(result, dict) else {}
        return {
            "mode": mode,
            "summary": "Leader is showing the read-only multi-plan board without mutating any plan or approval state.",
            "reason": "human asked to inspect every plan and its gate",
            "next_command": next_command,
            "recommended_action_id": None,
            "action_kind": "plan_board",
            "action_status": plan_board_card.get("mode"),
            "safety": "inspect",
            "requires_explicit_user": False,
        }
    ...
    if mode == "role_topology":
        ...
        return {
            ...
            "next_command": next_command,
            "recommended_action_id": None,
            "action_kind": "role_topology",
            "action_status": role_topology_card.get("mode"),
            "safety": "inspect",
            "requires_explicit_user": False,
        }
    ...
    if mode == "memory_suggestions":
        ...
        return {
            "mode": mode,
            "summary": "Leader is showing pending memory suggestions without writing long-term memory or calling a provider.",
            "reason": "human asked to inspect pending memory suggestions",
            "next_command": next_command,
            "recommended_action_id": None,
            "action_kind": "memory_suggestions",
            "action_status": "pending" if int(suggestions_card.get("pending_count", 0)) else "empty",
            "safety": "inspect",
            "requires_explicit_user": False,
        }
...
def _chat_wants_plan_board(message: str) -> bool:
    text = message.strip()
    return bool(
        re.search(r"(查看所有计划|计划看板|所有计划|查看计划列表|计划总览|计划列表)", text, re.IGNORECASE)
        or re.search(r"\bplan board\b", text, re.IGNORECASE)
    )
...
def leader_chat_command(args: argparse.Namespace) -> int:
    ...
    if _chat_wants_plan_board(args.message):
        plan_board_card = _plan_board_payload(store)
        next_command = "agentdeck plan board"
        turn = store.record_chat_turn(
            mode="plan_board",
            message=args.message,
            plan_id=None,
    ...
    if _chat_wants_skill_suggestions(args.message):
        ...
        skill_suggestions_control_id = next(
            (
                item.get("control_id")
                for item in registry_items
                if isinstance(item, dict)
                and item.get("scope") == "skills"
                and item.get("card") == "skill_suggestions_card"
        ...
        control_registry_card = leader_chat_control_registry_card(
            ...
            scope="skills",
            card="skill_suggestions_card",
            control_id=str(skill_suggestions_control_id) if skill_suggestions_control_id else None,
        )
        payload = {
            "ok": True,
            "turn_id": turn["turn_id"],
    ...
    if _chat_wants_memory_context(args.message):
        ...
        workbench_card = _workbench_snapshot_payload(refreshed_project_view, store, since_event_id=None)
        registry_items = workbench_card.get("control_registry") if isinstance(workbench_card.get("control_registry"), list) else []
        memory_context_control_id = next(
            (
                item.get("control_id")
                for item in registry_items
                if isinstance(item, dict)
                and item.get("scope") == "memory"
                and item.get("card") == "memory_context_card"
                and item.get("kind") == "inspect"
                and item.get("command") == next_command
            ),
            None,
        ...
        control_registry_card = leader_chat_control_registry_card(
            ...
            scope="memory",
            card="memory_context_card",
            control_id=str(memory_context_control_id) if memory_context_control_id else None,
        )
        payload = {
            "ok": True,
            "turn_id": turn["turn_id"],
    ...
    if _chat_wants_memory_suggestions(args.message):
        next_command = "agentdeck memory suggestions"
        turn = store.record_chat_turn(
            mode="memory_suggestions",
            message=args.message,
        ...
        workbench_card = _workbench_snapshot_payload(refreshed_project_view, store, since_event_id=None)
        registry_items = workbench_card.get("control_registry") if isinstance(workbench_card.get("control_registry"), list) else []
        memory_suggestions_control_id = next(
            (
                item.get("control_id")
                for item in registry_items
                if isinstance(item, dict)
                and item.get("scope") == "memory"
                and item.get("card") == "memory_suggestions_card"
                and item.get("kind") == "inspect"
                and item.get("command") == next_command
            ),
            None,
        ...
        control_registry_card = leader_chat_control_registry_card(
            ...
            scope="memory",
            card="memory_suggestions_card",
            control_id=str(memory_suggestions_control_id) if memory_suggestions_control_id else None,
        )
        payload = {
            "ok": True,
            "turn_id": turn["turn_id"],
            ...
            "leader_explanation": _leader_chat_explanation(
                ...
                next_command=next_command,
                project_view=refreshed_project_view,
                result=memory_suggestions_card,
            ),
            "plan_id": None,
            "review": None,
            "recovery": refreshed_project_view.get("recovery"),
    ...
    if _chat_wants_artifacts(args.message) and _chat_task_assignment_intent(args.message, config) is None:
        ...
        if not validation["ok"]:
            ...
            for error in validation["errors"]:
                print(f"- {error}", file=sys.stderr)
            return 1
        registry_items = _workbench_control_registry({"artifacts_card": artifacts_card})
        artifacts_control_id = next(
            (
                item.get("control_id")
                for item in registry_items
                if isinstance(item, dict)
                and item.get("scope") == "artifacts"
                and item.get("card") == "artifacts_card"
                and item.get("kind") == "inspect"
                and item.get("command") == next_command
            ),
            None,
        ...
        control_registry_card = leader_chat_control_registry_card(
            ...
            scope="artifacts",
            card="artifacts_card",
            control_id=str(artifacts_control_id) if artifacts_control_id else None,
        )
        payload = {
            "ok": True,
            "turn_id": turn["turn_id"],
    ...
    if _chat_wants_role_topology(args.message):
        next_command = "agentdeck workbench"
        turn = store.record_chat_turn(
            mode="role_topology",
            message=args.message,
        ...
        if refreshed_project_view is None:
            return 1
        role_topology_card = _workbench_role_topology_card(
            refreshed_project_view, _workbench_review_gate_card(refreshed_project_view)
        )
        registry_items = _workbench_control_registry({"role_topology_card": role_topology_card})
        role_topology_control_id = next(
            (
                item.get("control_id")
                for item in registry_items
                if isinstance(item, dict)
                and item.get("scope") == "role_topology"
                and item.get("card") == "role_topology_card"
                and item.get("kind") == "inspect"
        ...
        control_registry_card = leader_chat_control_registry_card(
            ...
            scope="role_topology",
            card="role_topology_card",
            control_id=str(role_topology_control_id) if role_topology_control_id else None,
        )
        payload = {
            "ok": True,
            "turn_id": turn["turn_id"],
...
def build_parser() -> argparse.ArgumentParser:
    ...
    protocol_acp_load = protocol_acp_subparsers.add_parser("load", help="Explicitly load an ACP session")
    protocol_acp_load.add_argument("--session-id", required=True)
    protocol_acp_load.add_argument("--confirm", action="store_true")
    protocol_acp_load.set_defaults(func=protocol_acp_load_command)
    protocol_acp_resume = protocol_acp_subparsers.add_parser("resume", help="Explicitly resume an ACP session and prompt")
    protocol_acp_resume.add_argument("--session-id", required=True)
    protocol_acp_resume.add_argument("--prompt", required=True)
    protocol_acp_resume.add_argument("--confirm", action="store_true")
    protocol_acp_resume.set_defaults(func=protocol_acp_resume_command)
