...
def validate_leader_chat_contract(payload: dict[str, object]) -> dict[str, object]:
    ...
    if isinstance(runtime_action_card, dict):
        ...
        if explanation_action_kind in {"runtime_send", "runtime_stop", "runtime_refresh"} and runtime_action_card.get("command") != payload.get("next_command"):
            errors.append("runtime_action_card.command must match response next_command")
    elif explanation_action_kind in {"runtime_send", "runtime_stop", "runtime_refresh"}:
