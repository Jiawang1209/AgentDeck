...
- 保存后的 plan、ProjectView `plans.items[]`、`agentdeck plan list` 和 run cards 必须暴露 `provider_backend`，取值为 `local`、`api`、`cli` 或 `unknown`；该字段只表示 plan provenance，方便 GUI/审计区分 fake/API-backed/CLI-backed Leader 来源，不得作为授权、dispatch 或 runtime 执行依据。
