...
- `agentdeck status` 的 `recovery` 是默认恢复入口，必须保持只读，并暴露 status/reason/next_command/recommended_action/pending/leader_action/latest_event/recent_events；`recommended_action` 必须说明 label/command/safety/requires_explicit_user/source。
...
- `agentdeck leader chat --message <text>` 是当前自然语言入口 MVP：无 plan 时创建 plan-only 记录，有 plan 时 review 最新 plan，并持久化或复用一条 `leader_actions[]` 建议；review 输出必须包含 `recovery`，且 `next_command` 必须来自 `status.recovery.next_command`；每轮会写入 `chat_turns[]`，可用 `agentdeck leader chat-history` 查看。
...
- `agentdeck leader next` 只把下一步建议写入 `leader_actions[]`，不得直接执行 action；相同 pending action 已存在时会复用原 action_id；执行仍需人类显式运行对应 approval/dispatch/capture 命令。
