...
- `agentdeck leader chat --message "追踪 msg_xxx"` 这类 direct trace 意图必须走只读 `mode=trace`，复用 `agentdeck trace --id <id>` 的 response shape 返回 `trace_card`；该模式只记录 chat turn，不得创建 plan/leader action，不得执行 ack、dispatch、capture reply 或发送 tmux 输入；嵌入的 `trace_card` 必须通过 `validate_trace_contract()` 校验，未知 trace id 必须返回错误且不创建 chat turn 或 plan。
