...
- `agentdeck leader chat --message "派发当前审批"` 必须继续保持只读 `mode=approval`，但当存在 approved approval 时应嵌入 `dispatch_preview_card`，展示目标 agent/role/pane/task/dispatch_command/inbox_command 和 blocker；它只提供 explicit-runtime 执行前预览，不创建 message/job/inbox、不发送 tmux 输入。
