...
- `agentdeck controls` / workbench `control_registry[]` 必须保留 `scope=terminal_session` 的 `kind=attach_session` / `kind=open_controls` / `kind=refresh_runtime` 项目级终端入口，以及 runtime scope 的 `kind=terminal` item，供 GUI/TUI 直接渲染终端工具条；attach/open 是 inspect-only，refresh_runtime 是 explicit_runtime，不代表自动 attach tmux、select pane、capture pane、send input 或写 state 的许可。
