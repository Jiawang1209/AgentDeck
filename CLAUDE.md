...
- `agentdeck leader chat --message "查看队列"` / `"查看控制面"` 必须进入只读 `mode=queue`，嵌入 workbench 同源 `queue_card` / `operator_card` 并展示 next/apply/explicit controls；顶层 `next_command` 必须对齐 `operator_card` 的主命令，因此多条 approved approvals 时应推荐 `agentdeck approval dispatch-ready --confirm`；它不创建或应用 action、不审批、不派发、不 ack、不 refresh runtime、不发送 tmux 输入。
