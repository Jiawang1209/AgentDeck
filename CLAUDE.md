...
- `agentdeck leader chat --message "继续"` 在 recovery source 为 `runtime` 时必须嵌入 `runtime_card`，复用 workbench runtime card 字段规则；在 recovery source 为 `reply` / `reply_waiting` 时必须嵌入同源 `trace_card`，复用 `agentdeck trace --id <message_id>` 的通信 lineage 字段，并只建议显式 `agentdeck capture-reply --agent <id> --message-id <msg_id>`；它只展示恢复入口，不自动 refresh、spawn、stop、capture、capture-reply 或发送 tmux 输入。
