...
- `agentdeck leader chat --message "查看运行进度 <plan_id>"` 必须进入只读 `mode=run_progress`，复用 `agentdeck run --plan-id <plan_id>` 的 `run_progress_card` 契约；它只可记录 chat turn，不得修改 plan/approval/runtime state，不得创建 `leader_actions[]`，不得 approve、dispatch、capture pane、ack inbox、创建 runtime message/job/inbox 或发送 tmux 输入；`intent_card.embedded_card=run_progress_card`，inspect control 必须指向 `agentdeck run --plan-id <plan_id>`。
