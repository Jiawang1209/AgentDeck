...
| `skills list/show/import-preview/import/load-preview/load` 与 plan `skill_context` provenance | 可审计、可回放、可被 GUI 消费的 Skill Layer |
...
- `agentdeck skills list` / `agentdeck skills show --name <name>` / `agentdeck skills import-preview --path <SKILL.md>` / `agentdeck skills import --path <SKILL.md>` / `agentdeck skills load-preview --name <name> --agent <id> --purpose <text>` / `agentdeck skills load --name <name>`。
