...
**待批证明(终审 2026-08-01 追加,原设计遗漏)。** 上面两条**不足以**
判定。`_detect_waiting_for_input` 的 marker `Would you like to run` 在
**已答复**的折叠框(`… ? -> Yes`)上同样命中,而命令框提取器不像 MCP
提取器那样要求选择器字形,于是一道早已答复的框可以:

- 产出**全 None 身份**(两个提取器都失败)——全 None 恒等于自身,
  debounce 必然确认,一个健康的走开段被误停;
- 或从残留 `$ ` 行刮出一条**从未对应过任何框**的命令,身份非空,
  照样误停,还把假命令递给人类。

两条都由终审端到端复现。因此再加两条判据:

3. `box_pending` —— 屏上确有待批框,判据是活动选择器字形 `›1.`
   (全空白折叠后,在挂起框区域内),与 `_MCP_TOOL_BOX_PATTERN` 已有的
   那道硬约束同源。待批框在正文正下方渲染 `› 1. Yes, proceed (y)`;
   已答复的折叠框后面是 `-> Yes` 之类的文本,永不含该字形。
4. `box_kind` 非空 —— 即本节安全边界里那条"解析不出框一律不判定"。
   它必须挡在**候选**阶段而非 debounce 阶段,因为全 None 身份自等。

`box_pending` 只是扫描附加字段,**不进** `HUMAN_GATE_FIELDS`(对确认的
人类门它恒为真,进证据对象只是噪声),也**不改变放行路径**——哪些框被
放行仍然只由精确委托匹配决定。
