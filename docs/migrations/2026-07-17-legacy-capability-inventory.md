...
| Skill/Memory/Learning structured records | `StateStore` currently owns `skill_loads`, skill and memory suggestion records and statuses, application provenance, and Learning Review/suggestion metadata in legacy JSON state. | refactor | ProjectDaemon-owned SQLite structured-state authority | P1 | `tests/test_agent_cli.py`, `tests/test_leader_cli.py`, and state-backed contract tests characterize load provenance, pending-to-created/applied suggestion transitions, read-only review behavior, and explicit confirmation. | Before `sqlite_active`, import and verify every structured record, status, lineage, and application/review provenance; after cutover, legacy JSON must be read-only compatibility input and never a second authority. Missing or unverifiable records fail the cutover closed. |
| Skill Registry, Memory source/context behavior, and Learning Review UX | `skills.py`, CLI handlers, and contract/workbench projections implement explicit import/load/create/apply controls, filesystem-owned Skill/Memory source text, and read-only Learning Review suggestions. | retain | P5 governed learning and improvement services using V1 Evidence and SQLite provenance | P5 | `tests/test_agent_cli.py` proves skill and memory suggestion queues plus read-only `learn review`; `tests/test_contracts.py` proves `learning_review` contract validation; `tests/test_leader_cli.py` proves natural-language Learning Review does not mutate suggestions. | Refactor and integrate the governed learning service/UX only after P1 has moved its structured records to the sole SQLite authority and V1 Mission Evidence can be referenced without raw prompt/output leakage. Preview/confirm gates remain explicit; Skill/Memory source text remains filesystem content rather than a database body. |
| Legacy `cli.py` command handlers | `src/agentdeck/cli.py` is a 19,266-line parser, renderer, daemon client, provider/runtime coordinator, and legacy direct-write surface. | compat | Thin CLI/Conversation clients over application services and ProjectDaemon protocol | P1 authority cutover; P2-P4 handler extraction | `tests/test_agent_cli.py`, `tests/test_leader_cli.py`, `tests/test_daemon_cli.py`, and structured-output tests characterize commands, JSON surfaces, safety controls, and natural-language routes. | Before `sqlite_active`, every structured mutator must delegate to the sole-writer ProjectDaemon or be explicitly fail-closed/disabled; no CLI local-write fallback may survive authority cutover. After that P1 gate, extract command families incrementally and delete a handler only when its exact public contract, focused tests, and CLI contract tests pass through the new path. |
...
| V1 Mission domain and ProjectView v2 | Current mission/semantic modules and ProjectView v1 contain useful facts, but no single V1 aggregate owns Project, Conversation, immutable MissionVersion, AuthorizationEnvelope, Task DAG, Attempt, AgentSession, Permission, Handoff, Evidence, and Verification. | missing | V1 domain services and v2 read-model projector | P1 | Existing mission, semantic-authority, daemon governance/recovery, and ProjectView contract tests provide characterization inputs, but `PROJECT_VIEW_SCHEMA_VERSION` is still `project-view/v1`. | Complete when typed transition tests cover terminal precedence, one-confirmation scope, bounded recovery, verification grades, complete provenance, and v1/v2 projection compatibility from the same SQLite authority. P2 consumes this completed domain/projection through the Conversation product; it does not first create either authority. |
...
provenance, including Skill/Memory/Learning load, suggestion, status, application,
and review records. It does not mean deleting filesystem-owned repository content,
...
V1 domain/ProjectView v2 while keeping characterization green; P2 then makes the
Conversation product the primary client; P3 supplies official Codex/Claude adapters and a
