...
- 支持 `agentdeck memory suggest --summary <summary> --rationale <rationale> --source <source>` / `agentdeck memory suggestions`，先只写 pending `memory_suggestions[]` 和审计事件，不创建 `.agentdeck/memory/*.md`。
...
- Memory 默认只读，不自动写长期记忆；当前先记录待审 `memory_suggestions[]`，只有显式 `memory apply --confirm` 才能写入 `.agentdeck/memory/*.md`。
