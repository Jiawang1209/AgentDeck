...
Fresh gates: real crash matrix repeated twice (`9 passed` each), acceptance/crash/recovery (`125 passed`), daemon suite (`897 passed`), and full suite (`2776 passed, 1 skipped`). Real Claude ACP foreground acceptance passed. Full real M2 rehearsal remains explicitly **BLOCKED**, not PASS: installed Codex/Claude CLI Leaders did not satisfy strict Mission plan JSON reliably, and the transport-only fallback reached real Claude ACP success plus recorded handoff before real Codex tmux stayed submitted without a correlated reply beyond the bounded window. No install/auth changes or disposable-process residue occurred.
