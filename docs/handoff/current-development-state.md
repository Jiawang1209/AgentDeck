...
Candidate next slices in this lane (each a contained, pure-render cut): a navigable TUI approvals/queue view (select an approval → see its command, like the palette view); a dashboard **Ledger/trace** or **Autonomous run history** section from `run_loop_advanced` events; render the autonomous/run-loop state in the TUI. Larger forks still open (ask the human): a full standalone GUI client; Skill Registry marketplace/allowlist; multi-plan/parallel orchestration; remote/MCP transport. Whatever is chosen must preserve human approval and keep every read-only surface read-only.
