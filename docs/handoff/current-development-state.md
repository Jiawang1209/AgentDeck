...
**Phase 3 M2 implementation Tasks 1–14 are integrated into `main`; the active goal is the approved Phase 3 M2c acceptance closure.** The formal design is `docs/superpowers/specs/2026-07-14-agentdeck-m2c-closure-design.md`, and the detailed TDD plan awaiting human review is `docs/superpowers/plans/2026-07-14-agentdeck-m2c-closure.md`. Both start from `docs/roadmap/product-north-star.md`: AgentDeck—not the Leader model, ACP, or tmux—owns frozen Mission authority, scheduling, governance, recovery, and audit. The implementation must first make Codex/Claude CLI Leader planning use one frozen-authority-aware native JSON Schema while preserving strict local validation and compact diagnostics, then prove the complete real implementation → review → revision → acceptance Mission across Claude ACP and Codex tmux, including disconnect/reconnect, explicit permissions, visible tmux, takeover/return-control, three compact handoffs, state agreement, and cleanup. Do not begin implementation until the plan is reviewed and the execution mode is selected.
...
Next: review `docs/superpowers/plans/2026-07-14-agentdeck-m2c-closure.md`, then choose Subagent-Driven execution (recommended) or inline `executing-plans`. Implementation must begin in the plan's isolated `codex/m2c-closure` worktree and follow its twelve TDD/commit boundaries. Preserve the full M2c `leader_schema` blocker until a frozen-commit real four-stage rehearsal passes; the passing two-step transport proof must not be promoted to four-stage acceptance. M3 brainstorming starts only after that gate. A2A Client/Server, remote daemon, global roaming, Workspace Client, system notifications, complete transcript persistence, automatic install/auth, Windows IPC, and terminal-emulator work remain out of scope.
...
Phase 0, Phase 1, Phase 2, Phase 3 M1, and Phase 3 M2 implementation Tasks 1–14 are complete and integrated into `main`. M1's final full suite was 1855 passed and 1 skipped; its deterministic and live Claude ACP/PTY evidence is `docs/validation/2026-07-13-phase3-m1-foreground-conversation.md`. M2's final full suite is `2928 passed, 1 skipped`; deterministic evidence and the two-step real transport PASS are recorded in `docs/validation/2026-07-13-phase3-m2-project-daemon.md`, while full four-stage M2c remains explicitly blocked at the real Leader schema gate. The approved closure design is `docs/superpowers/specs/2026-07-14-agentdeck-m2c-closure-design.md`; the detailed TDD implementation plan is `docs/superpowers/plans/2026-07-14-agentdeck-m2c-closure.md` and awaits human review plus execution-mode selection.
