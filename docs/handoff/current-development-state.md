...
The final M2 security closure also anchors every audit-journal read/append to the verified project state directory, rejects symlink/non-regular/replaced `events.jsonl` paths, preserves daemon/conversation/protocol outboxes when a journal drain fails, and revalidates the held `protocol-mutation.lock` inode after `flock`. A replacement race therefore fails closed before state mutation rather than splitting authoritative writers across two lock inodes.
