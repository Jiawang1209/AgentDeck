...
Fresh verification for this final review closure is 973 daemon/ACP/protocol tests passing with one skip and 2,643 full-suite tests passing with one skip. Compileall and `git diff --check` pass. The new detached-daemon governance acceptance also verifies endpoint and detached reaper cleanup.
