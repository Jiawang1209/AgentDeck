...
**Phase 3 M2 implementation Tasks 1–14 are integrated into `main`; the active goal is the approved Phase 3 M2c acceptance closure design.** The formal design is `docs/superpowers/specs/2026-07-14-agentdeck-m2c-closure-design.md`. It starts from `docs/roadmap/product-north-star.md`: AgentDeck—not the Leader model, ACP, or tmux—owns frozen Mission authority, scheduling, governance, recovery, and audit. The next implementation slice must first make Codex/Claude CLI Leader planning use one frozen-authority-aware native JSON Schema while preserving strict local validation and compact diagnostics, then prove the complete real implementation → review → revision → acceptance Mission across Claude ACP and Codex tmux, including disconnect/reconnect, explicit permissions, visible tmux, takeover/return-control, three compact handoffs, state agreement, and cleanup. The design is approved; a detailed TDD implementation plan still requires separate human review before code changes.
