...
First slice of that lane is **done**: `render_workbench_dashboard` (shared by `agentdeck dashboard` and the TUI overview via `tui.py`) now has a read-only **Control mode** section (`_render_control_mode`, `src/agentdeck/dashboard.py`) surfacing the ask/approve/autonomous gradient and the `approval auto` / `run-loop` command hints with enabled/blocked state. Test: `tests/test_dashboard.py::test_render_workbench_dashboard_shows_control_mode_and_autonomous_commands`.

Candidate next slices in this lane (each a contained, pure-render cut): a dashboard **Runtime** section (agents · role · status · pane from `runtime_card`); a navigable TUI approvals/queue view (select an approval → see its command, like the palette view); render the autonomous state in the TUI. Larger forks still open (ask the human): a full standalone GUI client; Skill Registry marketplace/allowlist; multi-plan/parallel orchestration; remote/MCP transport. Whatever is chosen must preserve human approval and keep every read-only surface read-only.
