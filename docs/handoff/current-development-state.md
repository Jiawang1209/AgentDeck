...
The final M2 security closure also anchors every audit-journal read to the verified project state directory and implements append as a bounded 64 MiB full-content temp/fsync/identity-check/atomic-replace transaction. Symlink, non-regular, and post-proof replacement races cannot redirect new bytes to a detached journal inode; daemon/conversation/protocol outboxes stay pending on failure and retry without duplicate events. New writers acquire an anchored state-directory flock before the legacy filename lock, then revalidate both the lock-file inode and project state anchor, so replacing `protocol-mutation.lock` after proof cannot split current writers or cause a lost update.
