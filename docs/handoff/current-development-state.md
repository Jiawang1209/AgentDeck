...
Leader planning failure truth is now compact and durable. CLI Leaders emit only typed allowlisted stages (`timeout`, `nonzero`, `json_parse`, `schema`, `cancelled`, `oversize`); Gateway/session propagation never persists raw stdout/stderr, prompts, argv, paths, or exception text. Failed/cancelled turns immediately commit their terminal transition plus `conversation_turn_terminal.stage`, allowing reconnect/acceptance clients to stop without waiting for a nonexistent Mission. Natural-language planning also uses `mission_intent` + `select_mission_agents` before the Leader call and supplies only the uniquely requested Worker collection; preview creation repeats the same authoritative selection, so a third configured but unrequested Worker is excluded from both prompt scope and confirmation authority.

Mixed-version state authority is closed at the remaining filename-replacement window. Each state/journal effect now proves the held legacy lock still names `protocol-mutation.lock` before and after I/O. State commit retains the exact displaced canonical descriptor and applies content+inode CAS; after drift it waits on the replacement filename lock and restores only if canonical still contains AgentDeck's exact install. An older in-place write is therefore recovered from the retained inode, while an already atomic older replacement is preserved and the current mutation fail-stops. Journal commit uses the same conditional recovery and keeps the outbox pending. Six focused race cases and the `963`-test daemon suite pass.
...
Fresh gates: crash/acceptance focused group repeated twice (`12 passed` each), acceptance/crash/recovery (`127 passed`), daemon suite (`946 passed`), and full suite (`2827 passed, 1 skipped`). The final fresh-project live M2 rehearsal is **PASS**: Codex CLI `0.131.0` with `gpt-5.5` produced and confirmed the frozen Mission; Claude Agent ACP `0.58.1` completed Worker A after two exact permission confirmations; only after its validated reply and recorded compact handoff did the daemon start Codex tmux Worker B. Both Workers succeeded, the Mission completed, a new bare client reconnected to the same daemon, and exact force-stop plus suffix-scoped cleanup left no endpoint, tmux socket, process, or disposable-project residue. No install/auth changes occurred.
...
Next: run final spec/quality review, then choose integration through the normal development-branch finish flow. The real Codex/Claude rehearsal is a live PASS; it does not broaden M2 into A2A Client/Server, remote daemon, global roaming, Workspace Client, system notifications, complete transcript persistence, automatic install/auth, Windows IPC, or terminal-emulator work, which remain out of scope.
...
Phase 0, Phase 1, Phase 2, Phase 3 M1, and Phase 3 M2 implementation Tasks 1–14 are complete. M1's final full suite was 1855 passed and 1 skipped; its deterministic and live Claude ACP/PTY evidence is `docs/validation/2026-07-13-phase3-m1-foreground-conversation.md`. M2's final full suite is `2827 passed, 1 skipped`; its deterministic and real Codex/Claude evidence is `docs/validation/2026-07-13-phase3-m2-project-daemon.md`. M2 now awaits final review and the user's integration choice.
