...
Task 14 adds a real-SIGKILL nine-boundary crash matrix and one disposable product acceptance with a bare `agentdeck` PTY reconnect, exact permission preview/confirm, ACP-before-tmux handoff ordering, compact ProjectView/contracts/ledger/events/hash/file agreement, and zero durable transcript/secret markers. Daemon ACP streamed chunks now persist only canonical content hash and byte count; the raw bounded fragments remain process-local until canonical handoff extraction.

Fresh gates: focused acceptance/crash repeated twice (`10 passed` each), acceptance/crash/recovery (`117 passed`), daemon suite (`889 passed`), and full suite (`2757 passed, 1 skipped`). Real Claude ACP foreground acceptance passed. Full real M2 rehearsal remains explicitly **BLOCKED**, not PASS: installed Codex/Claude CLI Leaders did not satisfy strict Mission plan JSON reliably, and the transport-only fallback reached real Claude ACP success plus recorded handoff before real Codex tmux stayed submitted without a correlated reply beyond the bounded window. No install/auth changes or disposable-process residue occurred.
