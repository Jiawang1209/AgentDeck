...
Fresh verification for this final review closure is 240 focused governance/service/transport/recovery/CLI tests passing, 993 daemon/ACP/protocol tests passing with one skip, and 2,663 full-suite tests passing with one skip. Compileall and `git diff --check` pass. The detached-daemon governance acceptance verifies endpoint/reaper cleanup, and the in-process real RPC acceptance proves post-commit force-stop cleanup failure still returns accepted/stopping before the endpoint exits.
