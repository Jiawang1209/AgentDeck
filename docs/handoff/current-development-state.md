...
**Phase 3 M2 implementation Tasks 1–14 are complete and awaiting final review/integration.** The project-local authoritative daemon now owns frozen Mission scheduling after client disconnect, exact ACP/tmux supervision, compact handoff ordering, recovery, permission/ownership/reroute/shutdown governance, deterministic bare-CLI reconnect, and explicit existing-project migration. The implementation plan is `docs/superpowers/plans/2026-07-13-agentdeck-project-daemon.md`; deterministic and real-component evidence is `docs/validation/2026-07-13-phase3-m2-project-daemon.md`.

Task 14 adds a real-SIGKILL nine-boundary crash matrix and one disposable product acceptance whose first bare `agentdeck` PTY creates a natural-language Mission preview and confirms that exact preview, then disconnects; a second bare PTY reconnects at the ACP permission pause, renders recovery, and drives exact permission preview/confirm. The run proves ACP-before-tmux handoff ordering, compact ProjectView/contracts/ledger/events/hash/file agreement, and zero durable transcript/secret markers. Daemon ACP streamed chunks now persist only canonical content hash and byte count; the raw bounded fragments remain process-local until canonical handoff extraction.

Recovery truth is deliberately conservative: daemon loss after tmux dispatch but before receipt, and daemon loss while ACP permission is pending, classify the active attempt as `ambiguous` to prevent replay; the permission record remains pending for inspection. Force-stop persists Mission `interrupted` while retaining an active unknown external effect as attempt `ambiguous`. Status/run/workbench now expose the same fail-closed five-field `daemon_admission` provenance, so admitted daemon Missions do not fabricate a legacy `workflow_run_id`.

Fresh gates: real crash matrix repeated twice (`9 passed` each), acceptance/crash/recovery (`124 passed`), daemon suite (`896 passed`), and full suite (`2765 passed, 1 skipped`). Real Claude ACP foreground acceptance passed. Full real M2 rehearsal remains explicitly **BLOCKED**, not PASS: installed Codex/Claude CLI Leaders did not satisfy strict Mission plan JSON reliably, and the transport-only fallback reached real Claude ACP success plus recorded handoff before real Codex tmux stayed submitted without a correlated reply beyond the bounded window. No install/auth changes or disposable-process residue occurred.
...
Next: run final spec/quality review, then choose integration through the normal development-branch finish flow. Do not reinterpret the real rehearsal blocker as a live PASS. A2A Client/Server, remote daemon, global roaming, Workspace Client, system notifications, complete transcript persistence, automatic install/auth, Windows IPC, and terminal-emulator work remain out of scope.
