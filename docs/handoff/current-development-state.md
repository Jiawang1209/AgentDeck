...
**Next Best Step (single remaining GUI-mainline follow-up):** Add a natural-language `leader chat` intent for `run-loop` (e.g. "推进计划 pln_xxx" → read-only preview that suggests the explicit `run-loop` command; out of scope in the run-loop design). This is the more user-tangible but larger/riskier direction — the `leader_chat` dispatch is huge with strict intent/validator machinery (`intent_card`, per-mode validators, contract + docs sync). Preserve human approval; keep the preview read-only.
