...
1. Workbench `skills_catalog_card` (browse the configured `allowed_sources` with no argument) + natural-language "浏览技能源" intent (surface the catalog on the one-screen workbench and via `agentdeck leader chat`, same read-only card).
