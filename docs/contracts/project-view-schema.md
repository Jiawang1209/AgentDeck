...
Field list constants are also defined in `src/agentdeck/contracts.py`: `PROJECT_VIEW_TOP_LEVEL_FIELDS`, `PROJECT_VIEW_LEADER_FIELDS`, `PROJECT_VIEW_COORDINATION_ROLE_FIELDS`, `PROJECT_VIEW_MISSIONS_FIELDS`, `PROJECT_VIEW_MISSION_ITEM_FIELDS`, `PROJECT_VIEW_PLAN_ITEM_FIELDS`, `PROJECT_VIEW_LEADER_GENERATION_FIELDS`, `PROJECT_VIEW_RECOVERY_FIELDS`, `PROJECT_VIEW_RECOMMENDED_ACTION_FIELDS`, `PROJECT_VIEW_SKILLS_FIELDS`, `PROJECT_VIEW_SKILL_ITEM_FIELDS`, `PROJECT_VIEW_MEMORY_FIELDS`, `PROJECT_VIEW_MEMORY_ITEM_FIELDS`, `PROJECT_VIEW_MESSAGE_ITEM_FIELDS`, `PROJECT_VIEW_JOB_ITEM_FIELDS`, `PROJECT_VIEW_REPLY_ITEM_FIELDS`, and `PROJECT_VIEW_ARTIFACT_ITEM_FIELDS`.
...
`plans.items[].planner_backend`, `plans.items[].orchestrator_backend`, and `plans.items[].planner_brief` are additive `project-view/v1` fields carrying G2 planner/orchestrator split provenance. Plans generated by the single-stage Leader path project `null` for all three. Plans generated with the split configured (`[leader.planner]` / `[leader.orchestrator]` config sub-sections, no explicit `--provider/--model` override) project: `planner_backend` and `orchestrator_backend` as the same normalized logical-Leader identity card shape as `leader_backend` (both keep `agent_id=leader`, `runtime_kind=logical_leader`, `pane_backed=false`), identifying which backend produced the macro brief and which expanded it into steps; and `planner_brief` as the compact frozen `planner-brief/v1` snapshot (`schema_version`, `goal`, `acceptance_criteria`, `risks`, `macro_steps`, `content_hash`) that the orchestrator stage consumed. The top-level `provider`/`model`/`leader_backend` of a split plan equal the orchestrator backend. These fields are audit/display provenance only — never readiness, dispatch authorization, or an approval bypass — and the trace plan projection carries the same three fields. The validator accepts object-or-null and rejects malformed backend identity or brief shapes.

`plans.items[].review_rounds` is an additive `project-view/v1` field: 只读,已追加的 review 迭代轮数,普通 plan 为 0. It is the maximum `round` marker among that plan's steps that carry `origin="review_iteration"`, derived by the same pure `plan_review_rounds()` helper the writer uses to gate `max_review_rounds`; the trace plan projection carries the same field. Plan steps themselves may additionally carry optional `origin`, `round`, `triggered_by_reply`, `review_group`, and `review_group_member` provenance (as seen in `agentdeck plan status` step items): `null` on every ordinary step. `origin`/`round`/`triggered_by_reply` are set only on steps appended by the review-iteration writer. `review_group` (1-based group number) and `review_group_member` (0-based position inside the group) are set only on steps produced by the deterministic `[review].reviewers` expansion — including the re-review group an iteration round appends, which carries both `origin="review_iteration"` and the group markers. `agentdeck plan status` step items additionally project a derived `depends_on` (ascending step numbers): an ordinary step depends on the one before it, review-group members share the step before the group's first member, and a step after a group depends on every member of it. It is derived by the pure `agentdeck.step_dag` module on read — never persisted, never authored by the Leader, no migration. `agentdeck plan status` step items also project `worktree_base_commit`: the commit that step's task worktree was created from, resolved at dispatch (`git worktree add` checks out exactly that ref, so it is a provable fingerprint of the tree the agent was handed); `null` for steps with no task worktree and for plans created before the field existed. It is a **recorded fact only** — comparing it against what the branch points at today needs git, which `StateStore` never runs, so `agentdeck plan status` carries the live comparison as a sibling `review_bindings` block built by the CLI instead. That block reports one entry per verdict-bearing step **that has a recorded base branch**, and only those: a review performed without a task worktree (a shared-workspace reviewer) has no branch to bind a verdict to, produces no entry, and is therefore **not checked** — the block never claims coverage it does not have. Each entry carries a closed three-state `state`: `match` (the reviewed commit is still where the branch points), `drift` (reviewed, then changed), or `unverifiable` with a closed `reason` of `not_recorded` / `branch_missing` / `no_git_repo`. **`unverifiable` must never be rendered as verified** — `drift: false` and `verified: false` both read like "fine", and only one of them is a claim the program actually established. A binding whose commit was never recorded (a plan predating this feature) reports `not_recorded` and deliberately does **not** raise the blocker: withholding there would stall every in-flight plan. That single fail-open is intentional and is the only one. These fields are provenance only — they never authorize rework, bypass approval, or imply a step is safe to dispatch.
...
`replies.items[]` includes `verdict`, an additive `project-view/v1` field carrying the optional G5 `review-verdict/v1` payload a review worker emitted via a `verdict: <single-line JSON>` reply line (`criteria[]` with per-criterion `pass|fail|unknown`, `overall`, optional `score`/`notes`). Replies without a verdict line project `null`; an invalid verdict never blocks reply ingestion — the reply is recorded without the field and a `review_verdict_invalid` audit event is appended, while valid verdicts append `review_verdict_recorded`. The trace reply projection carries the same field. A verdict is display/audit evidence only: it never changes gate, approval, merge, or summarize behavior.
...
`messages.items[]` also includes `worktree_path` and `worktree_branch`, the task-worktree provenance for worktree-mode dispatches: when the target agent's `workspace_mode` is `worktree` and the project is a real git repository, dispatch creates `.agentdeck/worktrees/<agent_id>/<message_id>/` on branch `agentdeck/<agent_id>/<message_id>` from the current HEAD, declares the working directory in the dispatch prompt, and records both fields plus a `worktree_created` event. When creation is not applicable or not possible, both fields are `null`; a worktree-mode dispatch that could not create its worktree additionally records an auditable `worktree_skipped` event with the reason (this is a recorded degradation, never a silent fallback). `messages.items[]` additionally carries `worktree_base_branch`: when a plan step's dispatch creates a worktree and an earlier step of the same plan already produced a task branch, the new worktree's branch starts from that earlier branch tip (latest earlier step wins) and the base branch is recorded here — this is how a review step checks out the implementing step's work in its own isolated worktree (decision D). It is `null` for first steps, plan-less dispatches, and non-worktree messages. These fields are provenance only — they grant no permissions and are never an execution authority.
...
Each `missions.items[]` row contains `mission_id`, `schema_version`, `user_message`, `status`, `stop_reason`, `can_start`, `can_resume`, `blockers`, `provider`, `model`, `leader_backend`, `plan_id`, `plan_hash`, nullable compact `semantic_authority`, `workflow_run_id`, `current_step`, `step_count`, `timeout_seconds`, compact `selected_agents` and `startup_actions`, timestamps, `daemon_admission`, and deterministic status/confirmation/resume commands. Every non-null Mission semantic card must have exactly one same-`plan_id` Plan card and the two compact values must be equal; a standalone semantic Plan remains a `preview`, but a semantic Mission may not be orphaned. Plan items, Mission items, and the workbench Mission card each receive a defensive copy of that equal compact value, so mutation through one returned surface cannot alter another. The workbench does not read durable state or recompute authority. Exact validators accept only built-in Plan/Mission summary and item containers and reject unknown nested fields, malformed or uppercase hashes, lifecycle/count drift, Plan/Mission/workbench disagreement, hostile container subclasses without invoking hooks, and semantic blocker claims derived from runtime blockers. This compact provenance is audit/display metadata only: it is not execution authorization, permission evidence, transport readiness, or an approval bypass.

`daemon_admission` is the durable project-daemon admission projection with exact fields `state`, `snapshot_hash`, `blocker`, `recovery_command`, and `updated_at`: unconfirmed Missions project `not_confirmed`, confirmed but unreachable/rejected daemon submission projects `confirmed_not_admitted` with an explicit recovery command, and successful exact-digest admission projects `admitted` with no blocker or recovery command. Projection rebuilds only that five-field allowlist and validates the complete type/state combination. A malformed or extra-field admission row is replaced by a deterministic `not_confirmed` sentinel and a fixed Mission blocker; raw values, unknown fields, and credentials are never forwarded. This fail-closed normalization is read-only, so status/workbench remain contract-valid without repairing state or appending events. It is scheduling provenance, not transport readiness or execution authorization. `can_resume` is true only for `stopped` or `interrupted` Mission state; commands are affordances and do not authorize execution.
...
Whenever the latest plan review is awaiting a dispatched reply, the recovery object also carries an optional read-only derived field `reply_file_ready` (bool): `true` when the file-channel reply `.agentdeck/replies/<message_id>.reply.txt` for the awaited message already exists on disk, `false` otherwise. The field travels with the awaited reply, not with one recovery status: it appears both when `recovery.status` is `reply_waiting` and when a higher-priority queue (for example `inbox_pending`) masks it, so GUI clients can still show that the worker has announced completion. It is a completion *signal* only — `next_command` keeps its status-appropriate command, and `agentdeck status` never captures, reads, or ingests the reply. The field is absent when no dispatched reply is awaited, and `PROJECT_VIEW_RECOVERY_FIELDS` (the required field set) is unchanged. These counts do not execute anything; they help GUI clients show unresolved Leader diagnostics, reply capture work, and runtime reconciliation work alongside approvals, inbox items, and action queue work.
