...
- `approval`: `active_queue_source` is `approval`, and `approval_card` reuses the `agentdeck approval list` queue contract. If the active approved approval cannot be dispatched because the target runtime is not running, `operator_card.blocker` and the explicit control blocker must expose that runtime blocker.
