...
`mission_card` is `null` when ProjectView has no Mission; otherwise it is derived only from `project_view.missions.latest_id/items` plus the safe configured tmux session name. It reuses the legacy Mission status projection and adds the canonical natural-language `confirmation_command`. Semantic authority is not projected through ProjectView or workbench in Task 8; that additive projection remains Task 10, so `mission_card` intentionally has no `semantic_authority` field yet. Its five controls expose confirm, resume, status, attach, and workbench actions; confirm is enabled only for an unblocked pending Mission, resume only for an unblocked stopped/interrupted Mission with either legacy M1 authority or complete daemon admission. An incomplete frozen/admission record disables foreground resume with the same ProjectView blocker. For an admitted Mission, invoking resume obtains a daemon-governance preview and the returned exact preview command is the only confirmation path. Rendering stays read-only: it does not inspect tmux, invoke a provider, execute a command, append an event, or mutate state.

The independent stable field list is exposed as `mission_card_fields`; in Task 8 it is the Mission status field tuple with `semantic_authority` explicitly excluded, plus `confirmation_command`. Task 10 may add that compact field only after ProjectView supplies the same source-of-truth projection. A disabled confirmation uses the first compact Mission blocker when one exists, falling back to the current status only when there is no blocker. This preserves actionable, sanitized readiness evidence without accepting nested blocker values.
...
`role_topology_card` (`mode=role_topology`) is derived from `leader.coordination_roles[]` and the same `worker_lifecycle_card` items; it is the read-only live role topology that unifies logical Leader roles (`frontdesk`, `planner`, `orchestrator`) and configured worker roles with per-role provider, lifecycle, status, blocker, and an inspect-only next-step control. Logical roles stay `runtime_kind=logical_role` / `pane_backed=false` / `pane_id=null` / `agent_id=null`; worker roles use `runtime_kind=worker_pane` and reuse the worker `lifecycle_stage`, except a worker whose `agent_id` owns a `review_gate_card` stage gets a review-aware `status`/`blocker` overlay (`reviewed` / `reviewing` / `blocked`). Rendering it never spawns, dispatches, captures, acks, releases, or writes state.
`roles_card` is a **different** card: it is the north-star role binding map (`mode=role_bindings`, `source_command=agentdeck roles`), byte-identical to `agentdeck roles` and validated by the same `validate_role_bindings_contract()` (error prefix `roles_card: `). It answers "which of the six north-star layers has this project filled in", not "what is each role doing right now" — see `docs/contracts/role-bindings-schema.md`. Its fields are discoverable through `roles_card_fields` / `roles_card_role_fields` / `roles_card_contract` on `agentdeck contract workbench`. It is read-only and feeds no `control_registry[]` scope.
...
`role_topology_card` (`mode=role_topology`, not to be confused with the `roles_card` / `role_bindings` contract) is the read-only live role topology projection: it unifies the logical Leader coordination roles (`frontdesk`, `planner`, `orchestrator`) and the configured worker roles into one ordered list so a GUI/TUI can show, at a glance, which role is thinking, waiting, executing, reviewing, or idle.
