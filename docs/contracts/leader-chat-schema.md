...
`frontdesk_card` is the read-only intake surface for messages such as `frontdesk <goal>`, `/frontdesk <goal>`, `前台接待 <goal>`, `梳理需求 <goal>`, or `澄清需求 <goal>`. These messages enter `mode=frontdesk`, keep `plan_id=null`, classify the intake as `planning_candidate` when goal text exists, and set `next_command` to the explicit planning command `agentdeck leader plan --task <goal>`. If no goal text is available, classification becomes `needs_goal` and the next command falls back to `agentdeck leader chat --message "帮助"`. The route records only a chat turn and audit event. It must not call a Leader provider, create a plan, create or apply a Leader action, create approvals, inspect tmux panes, create message/job/inbox runtime records, or send tmux input. It exists to keep the human-facing frontdesk layer separate from provider-backed planner/orchestrator work.

`frontdesk_card` also carries the deterministic multi-route classification: `candidates` is the ordered candidate list produced by the pure `agentdeck.frontdesk` module and `route` is the top candidate's route. Both are appended after the eight original fields, whose positions, semantics, and values are unchanged — `classification` is still only `planning_candidate` / `needs_goal`, and `next_command` still follows the original goal-text rule. Each candidate is `{route, label, command, confidence, rationale}` with `route` drawn from the closed enum `plan`, `run`, `status`, `help`, `skill`, `memory` and `confidence` from the closed enum `high`, `medium`, `low` (strong wording, weak cue, and the help fallback respectively); candidates are ordered by confidence descending with ties keeping the declared route order. Classification is pure text matching: it never reads state, plans, approvals, or tmux, and never calls a provider. `controls[]` keeps its original `inspect` and `plan` entries first and then appends one `kind=route` control per candidate, each reusing the safety of the command it points at (`plan_only` for `plan`, `approval_gated` for `run`, `inspect` otherwise); a control whose command still carries a placeholder ships disabled with a blocker. Candidate commands are suggestion text for a human to run, never authorization, and the frontdesk never executes one. The same classification is available without a chat turn through the fully read-only `agentdeck frontdesk --message <text>` command; see `docs/contracts/frontdesk-schema.md` for the standalone response shape and the division of labor between the two entrypoints.

`skill_context_card` is the read-only loaded-skill projection for messages such as `查看已加载技能`, `/skills`, `skills`, or `skill context`. These messages enter `mode=skill_context`, embed `project_view.skills.items[]`, set `next_command=agentdeck skills list`, and expose inspect controls for `agentdeck skills list` and `agentdeck status`. The same response includes `control_registry_card` filtered to `scope=skills` / `card=skill_context_card`; its selection points at the `agentdeck skills list` inspect control, and `intent_card.secondary_embedded_cards[]` lists `control_registry_card` so GUI shells can highlight the same loaded-skill command in the command palette. The route records only a chat turn and audit event. It must not call a Leader provider, inspect tmux panes, install or rewrite skills, create plans/actions/approvals/messages/jobs/inbox items, or change approval/runtime state.

`memory_context_card` is the read-only applied long-term memory projection for messages such as `查看长期记忆`, `长期记忆`, `已应用记忆`, or `memory context`. These messages enter `mode=memory_context`, embed `project_view.memory.items[]`, set `next_command=agentdeck status`, and expose inspect controls for `agentdeck status` and `agentdeck memory suggestions`. The same response includes `control_registry_card` filtered to `scope=memory` / `card=memory_context_card`; its selection points at the `agentdeck status` inspect control, and `intent_card.secondary_embedded_cards[]` lists `control_registry_card` so GUI shells can highlight the same memory context command in the command palette. The route records only a chat turn and audit event. It must not call a Leader provider, inspect tmux panes, read full memory content into the response, inject memory into Leader or Worker prompts, create plans/actions/approvals/messages/jobs/inbox items, or change approval/runtime state.
...
`skill_create_preview_card` is the natural-language wrapper around a pending skill suggestion for messages such as `创建 skill 建议 sgs_xxx`, `创建技能建议 sgs_xxx`, or `create skill suggestion sgs_xxx`. These messages enter `mode=skill_create_preview`, resolve the pending suggestion, and embed a read-only card with the proposed `SKILL.md` content, content hash, target project path, `draft_preview_command`, explicit `create_command`, and GUI-ready controls. The top-level `next_command` points at `agentdeck skills create --suggestion-id <id> --confirm`. The route may record only a chat turn and audit event; it must not create `SKILL.md`, mutate `skill_suggestions[]`, import or load skills, call a Leader provider, inspect tmux panes, create plans/actions/approvals/messages/jobs/inbox items, or change approval/runtime state. Unknown suggestion ids fail non-zero before any state mutation.

`skill_suggestions_card` is the read-only pending skill suggestion projection for messages such as `查看 skill 建议`, `查看技能建议`, or `skill suggestions`. These messages enter `mode=skill_suggestions`, embed the current `skill_suggestions[]` queue with `count`, `pending_count`, `items[]`, and inspect controls, set `next_command=agentdeck skills suggestions`, and expose `intent_card.embedded_card=skill_suggestions_card`. The same response includes `control_registry_card` filtered to `scope=skills` / `card=skill_suggestions_card`; its selection points at the `agentdeck skills suggestions` inspect control, and `intent_card.secondary_embedded_cards[]` lists `control_registry_card` so GUI shells can highlight the same suggestion-queue command in the command palette. The route may record only a chat turn and audit event; it must not create `SKILL.md`, import skills, load skills, call a Leader provider, inspect tmux panes, create plans/actions/approvals/messages/jobs/inbox items, or change approval/runtime state.

`memory_suggestions_card` is the read-only pending memory suggestion projection for messages such as `查看 memory 建议`, `查看记忆建议`, or `memory suggestions`. These messages enter `mode=memory_suggestions`, embed the current `memory_suggestions[]` queue with `count`, `pending_count`, `items[]`, `apply_preview_command_template`, item-level `apply_preview` / `apply_memory` controls, and inspect controls, set `next_command=agentdeck memory suggestions`, and expose `intent_card.embedded_card=memory_suggestions_card`. The same response includes `control_registry_card` filtered to `scope=memory` / `card=memory_suggestions_card`; its selection points at the `agentdeck memory suggestions` inspect control, and `intent_card.secondary_embedded_cards[]` lists `control_registry_card` so GUI shells can highlight the same memory suggestion queue command in the command palette. The route may record only a chat turn and audit event; it must not create or modify `.agentdeck/memory/*.md`, inject memory into prompts, call a Leader provider, inspect tmux panes, create plans/actions/approvals/messages/jobs/inbox items, or change approval/runtime state. Only a separate explicit `agentdeck memory apply --suggestion-id <id> --confirm` command may write long-term memory.
...
The real card also includes Leader scheduling entries for `frontdesk`, `plan`, `review`, and `apply_action`, plus `policy`, `provider_switch`, `skill_context`, `skill_import_preview`, `skill_load_preview`, `skill_create_preview`, `memory_context`, and `learning_review` entries for explicit configuration, skill lifecycle, loaded-skill inspection, applied-memory inspection, or learning-review commands. `frontdesk` uses `safety=inspect`, points at `agentdeck leader chat --message "frontdesk <goal>"`, advertises `frontdesk_card`, and keeps its placeholder control disabled until a GUI or human supplies goal text; the resulting frontdesk response may recommend `agentdeck leader plan --task <goal>`, but it does not run planning itself. `plan` must use `safety=plan_only` and its control command should point at the explicit `agentdeck leader plan --task <goal>` planning entrypoint; `review` and `apply_action` must use `safety=safe_apply`, with review pointing at `agentdeck leader review --plan-id <plan_id>` and apply_action pointing at `agentdeck leader apply-action --action-id <action_id>`; `policy` uses `safety=explicit_user` and points at `agentdeck policy set-mode --mode <mode>`; `provider_switch` uses `safety=explicit_user` and points at `agentdeck leader set-provider --provider <provider> --model <model>`; `skill_import_preview`, `skill_load_preview`, and `skill_create_preview` use `safety=explicit_user` because the suggested next command changes skill registry or replayable context, even though each preview route itself is read-only; `skill_context` uses `safety=inspect`, points at `agentdeck leader chat --message "查看已加载技能"`, and advertises `skill_context_card`; `memory_context` uses `safety=inspect`, points at `agentdeck leader chat --message "查看长期记忆"`, and advertises `memory_context_card`; `learning_review` uses `safety=inspect` because the route only displays the learning card and explicit suggestion commands. Read-only views such as `workbench`, `continue`, `leader_status`, `runtime`, `capture`, `role`, `ledger`, `trace`, `queue`, `approval`, `inbox`, and setup diagnostics use `safety=inspect` unless the downstream mode explicitly recommends an explicit runtime or provider configuration command. The `leader_status` capability points at `agentdeck leader status`, advertises `leader_status_card`, and lists `查看 Leader 状态`, `刷新 Leader 状态`, `Leader 概览`, `leader status`, `leader refresh`, and `leader overview` in `example_messages`, so GUI and natural-language shells can discover the same read-only status surface exposed by the direct route.

Every capability item includes `controls[]` using the same `kind`, `label`, `command`, `safety`, `enabled`, and `blocker` shape as intent controls; `agentdeck contract leader-chat` exposes the same shape as `capability_control_fields`. Direct read-only commands such as `agentdeck workbench` are enabled. Template commands may only use known placeholders: `<goal>`, `<plan_id>`, `<action_id>`, `<agent_id>`, `<mode>`, `<provider>`, `<model>`, `<SKILL.md>`, `<name>`, `<purpose>`, or `<suggestion_id>`. `capability_placeholders[]` exposes that whitelist with the matching blocker for each placeholder so GUI clients can render template inputs without parsing this Markdown document. Template commands with those placeholders must be disabled and must include a matching blocker such as `requires goal text`, `requires plan_id`, `requires action_id`, `requires agent_id`, `requires control mode`, `requires leader provider`, `requires leader model`, `requires SKILL.md path`, `requires skill name`, `requires purpose`, or `requires suggestion_id`. Capability controls must keep `command` and `safety` aligned with their parent capability item.
...
When continue-mode recovery points at a pending inbox item, the response also includes that agent's `inbox_card` and a filtered `control_registry_card` for the same inbox controls; when recovery points at approvals, the response also includes `approval_card`; when more than one approval is already approved, the embedded `continue_card.next_command` may be promoted to `agentdeck approval dispatch-ready --confirm`; when recovery points at stale runtime, the response also includes same-snapshot `runtime_card` and `terminal_session_card`, while `intent_card.embedded_card` remains `continue_card` and `intent_card.secondary_embedded_cards` lists `runtime_card` and `terminal_session_card`; when recovery points at `reply_waiting`, the response may include `trace_card` for the pending message lineage while still recommending the explicit `agentdeck capture-reply --agent <id> --message-id <id>` command. In that reply-waiting case, `intent_card.embedded_card` should prefer `trace_card` over `continue_card`, so GUI shells can render the actual communication evidence as the primary card and still use the next control for capture-reply. The top-level response `next_command`, `leader_explanation.next_command`, and persisted chat turn `next_command` must match `continue_card.next_command`. Continue-mode remains read-only: embedded cards and registry companions are display/action affordances, not automatic ack, approve, dispatch, capture-reply, refresh, spawn, stop, attach/select-pane, or tmux input.
...
When `leader_summary_card` is present, `validate_leader_chat_contract()` reuses `validate_leader_summary_contract()` and prefixes nested errors with `leader_summary_card:`. Live summary-mode responses also include a `control_registry_card` filtered to `scope=leader_summary` / `card=leader_summary_card`, derived from `leader_summary_card.controls[]`; its selection must point at the `kind=summary` control whose command matches the top-level `next_command`. `intent_card.secondary_embedded_cards[]` lists `control_registry_card` so GUI clients can render the deterministic summary card and command-palette selection together. Summary-mode records a chat turn for history, but it must not create Leader actions, approvals, messages, jobs, replies, artifacts, or inbox items; it must not call a provider, inspect tmux panes, capture replies, dispatch work, acknowledge inbox items, or send tmux input. The top-level `next_command` remains `agentdeck leader summary --plan-id <id>`, and `intent_card.embedded_card` should be `leader_summary_card`.
...
When the human asks to open or focus a specific visible agent terminal, terminal-mode embeds `terminal_card`, which reuses `agentdeck agent terminal --agent <id>` and includes tmux attach/select-pane commands plus capture/send/stop/inbox controls. The top-level `next_command` matches `terminal_card.attach_command`. Live terminal-mode responses also include a `control_registry_card` filtered to `scope=terminal` / `card=terminal_card`; its selection points at the enabled `kind=terminal` control whose command matches the top-level tmux attach command, and `intent_card.secondary_embedded_cards[]` lists `control_registry_card`. This remains a command surface only; chat does not attach to tmux, select panes, capture output, send input, stop panes, or write runtime state.
...
Artifacts-mode responses are returned when the human asks to inspect worker artifacts, outputs, deliverables, or `产物`. Explicit task assignment takes precedence: when the message also matches the task-assignment pattern (`让`/`指派`/`ask`/`assign` + a known agent id + a non-empty task, e.g. a task whose text merely contains an `.agentdeck/artifacts/...` path), the response must route to `mode=approval` task assignment instead of read-only artifacts mode. Otherwise they return `artifacts_card`, reusing the same shape as `agentdeck artifacts`:
...
When `role_card` is present, `validate_leader_chat_contract()` checks the same role card and role agent field lists exposed by `agentdeck contract workbench`. Role-mode responses also include a `control_registry_card` filtered to `scope=role` / `card=role_card`, derived from `role_card.agents[].controls[]`, so GUI clients can render the same role-edit form controls from chat and workbench. Role-mode records a chat turn for history, but it must not edit `.agentdeck/config.toml`, create plans/actions/approvals/messages/jobs/inbox items, or send tmux input.

Natural-language role assignment, such as `把 planner 设为 架构师` or `set reviewer role to QA`, also returns `mode=role`; however, `next_command` is a concrete `agentdeck agent assign-role --agent <id> --role <role> --role-prompt <prompt>` command, `leader_explanation.action_kind` is `role_assign`, and the `intent_card.controls[]` next control uses label `Assign role`, `safety=explicit_user`, and `requires_explicit_user=true`. The companion registry selection points at the target agent's disabled `kind=assign_role` template control; because that template still contains placeholders, `selection.next_command` remains `null` and the executable command stays in the top-level `next_command`. This intent suggests the explicit command only; it must not mutate role config automatically.
...
Natural-language task assignment, such as `让 planner 规划 README 更新` or `ask reviewer to review docs`, also returns `mode=approval`. In this case AgentDeck creates exactly one pending approval with `source=leader_chat_task_assignment`, embeds the refreshed `approval_card`, sets `leader_explanation.action_kind` to `approval_create`, and points `next_command` at the new approval's `approve_command`. Task assignment takes precedence over generic keyword sniffing: when a message matches the assignment pattern (`让`/`请让`/`安排`/`指派`/`ask`/`assign` + a known agent id + a non-empty task), the broad substring-based read-only intents — leader status (`leader`+`状态`), summary (`总结`/`汇总`), setup (`检查`/`provider`), workbench (`总览`/`控制台`), audit (`审计`/`事件`), ledger (`通信`/`链路`), and artifacts (`artifacts`/`产物`) — must not hijack the route; the response must still be the `mode=approval` task assignment above. Exact-phrase intents (help, skill/memory context, review gate, release preview, role topology) keep their existing matching because they cannot collide with a well-formed assignment sentence. The response's `intent_card.read_only` is `false` because the approval queue changed, but runtime work is still gated: it must not approve, dispatch, create messages/jobs/inbox items, or send tmux input.
...
- Chat run-progress responses must embed `run_progress_card`, reusing the `agentdeck run --plan-id <id>` response shape and `validate_run_start_contract()`. They also include a `control_registry_card` filtered to `scope=run_progress` / `card=run_progress_card`, derived from `run_progress_card.controls[]`; its selection points at the `kind=plan_status` inspect control for the same plan so GUI clients can highlight the progress-card entry while leaving the top-level `next_command` free to represent the actual next run action. Natural-language progress intents such as `查看运行进度` use the latest plan by default; `查看运行进度 pln_xxx` inspects the specified plan. If no plan exists, the command must fail without creating a plan, action, approval, message, job, inbox item, or chat turn. Successful run-progress responses are read-only: they may record a chat turn, but must not mutate plan/approval/runtime state, create Leader actions, dispatch, capture pane output, acknowledge inbox items, create message/job/inbox runtime records, or send tmux input.
- Chat run-loop-preview responses (`mode=run_loop_preview`, triggered by phrasings such as `推进计划 pln_xxx`, `往前推`, `驱动计划`, `run-loop`) embed a read-only `run_loop_preview_card` with fields `mode`, `plan_id`, `command`, `autonomous_enabled`, `safety`, `requires_explicit_user`, `blocker`, `enable_command`, `controls`. The card `command` is the explicit `agentdeck run-loop --plan-id <id> --confirm`; the top-level `next_command` must equal it and `intent_card.embedded_card` must be `run_loop_preview_card`. The intent next control uses `safety=explicit_runtime` (never inspect, per the requires-explicit-user rule) and is enabled only when `autonomous_enabled` is true; when autonomous mode is off it is disabled with blocker `autonomous mode is not enabled` and the card surfaces the `enable_command`. The response also embeds a `control_registry_card` filtered to `scope=autonomous` / `card=control_mode_card`, whose selection points at the disabled `run_loop` template control (the concrete filled command lives only in the top-level `next_command` and the intent next control — the `role_assign` pattern). This intent requires a plan id (no guessing); a run-loop phrasing with no `pln_...` returns non-zero without writing anything, and an unknown plan id returns an error. The response is read-only: it records only the chat turn and a `leader_chat_turn` audit event, and must not call a provider, read/write tmux, auto-approve, dispatch, or mutate approval/runtime/plan state. Executing the run-loop wave remains an explicit human command.
- Chat plan-board responses (`mode=plan_board`, triggered by phrasings such as `查看所有计划`, `计划看板`, `所有计划`, `查看计划列表`, `计划总览`, `plan board`) embed a read-only `plan_board_card` that reuses the `agentdeck plan board` response shape (`_plan_board_payload`) and is validated by `validate_plan_board_contract()` (errors prefixed `plan_board_card: `). The card shows every plan with its gate and next command; no plan id is parsed. The top-level `next_command` must equal `agentdeck plan board`, `intent_card.embedded_card` must be `plan_board_card`, `leader_explanation.action_kind=plan_board`, `safety=inspect`, and `requires_explicit_user=false`. The intent next control is a `kind=next` inspect control (allowed because `requires_explicit_user` is false), and there is no `scope=plan_board` command registry, so `control_registry_card` is `null` (mirroring the plan/review modes). The response is read-only: it records only the chat turn and a `leader_chat_turn` audit event, and must not call a provider, read/write tmux, or mutate plan/approval/runtime state.
- Chat skills-catalog responses (`mode=skills_catalog`, triggered by phrasings such as `浏览技能源`, `查看技能源`, `技能源`, `技能市场`, `技能目录`, `skill sources`, `skill catalog`) embed a read-only `skills_catalog_card` that reuses the workbench skills-catalog card shape (`_skills_catalog_card(config)`, fields `mode`, `source_count`, `total_skill_count`, `imported_count`, `sources[]`). The card is no-argument: it overviews the configured `[skills] allowed_sources` skill sources. The top-level `next_command` must equal `agentdeck skills sources`, `intent_card.embedded_card` must be `skills_catalog_card`, `leader_explanation.action_kind=skills_catalog`, `safety=inspect`, and `requires_explicit_user=false`. The intent next control is a `kind=next` inspect control to `agentdeck skills sources` (allowed because `requires_explicit_user` is false), and there is no `scope=skills_catalog` command registry, so `control_registry_card` is `null` (mirroring the plan_board mode). The response is read-only: it records only the chat turn and a `leader_chat_turn` audit event, and must not call a provider, read/write tmux, import/load skills, or mutate plan/approval/skill/runtime state.
- Chat frontdesk responses must embed `frontdesk_card`, use `leader_explanation.action_kind=frontdesk`, and keep `intent_card.read_only=true`. They may recommend `agentdeck leader plan --task <goal>` as the next command, but they must not call a provider or create a plan.
...
- Chat skill-import-preview responses must embed `skill_import_preview_card`, set `intent_card.embedded_card=skill_import_preview_card`, use `leader_explanation.action_kind=skill_import_preview`, and mark `safety=explicit_user` / `requires_explicit_user=true`; they are read-only previews and must not import, force-overwrite, load, or enable the skill until a human explicitly runs the suggested command.
...
- Chat terminal-mode responses must embed `terminal_card`, using the leader-chat `terminal_card_fields` for the requested visible agent pane, use `Open terminal` as the action-specific next label, include a `control_registry_card` filtered to `scope=terminal` / `card=terminal_card`, and select the `kind=terminal` tmux attach control whose command matches the top-level `next_command`.
...
- Chat review-gate responses must reuse the workbench `review_gate_card`, recommend `agentdeck workbench`, include a `control_registry_card` filtered to `scope=review_gate` / `card=review_gate_card`, list `control_registry_card` in `intent_card.secondary_embedded_cards`, and select the inspect control while also exposing disabled `assign_code_reviewer` / `assign_round_reviewer` templates for GUI role-configuration forms. They remain read-only except for recording the chat turn and its audit event. They must not release, merge, ack inbox items, dispatch follow-up work, advance the loop, call a provider, read/write tmux, or execute assignment templates.
- Chat release-preview responses must reuse the workbench `release_preview_card` derived from the same review gate, recommend `agentdeck workbench`, include a `control_registry_card` filtered to `scope=release_preview` / `card=release_preview_card`, list `control_registry_card` in `intent_card.secondary_embedded_cards`, and select the `inspect_review_gate` control. The `release_preview` / `next_round_preview` controls stay disabled `explicit_user` placeholders with `command=null` until an explicit approval-gated release command exists. They remain read-only except for recording the chat turn and its audit event. They must not release, merge, ack inbox items, dispatch follow-up work, advance the loop, call a provider, or read/write tmux.
- Chat role-topology responses must reuse the workbench `role_topology_card`, recommend `agentdeck workbench`, include a `control_registry_card` filtered to `scope=role_topology` / `card=role_topology_card`, list `control_registry_card` in `intent_card.secondary_embedded_cards`, and select the card-level `agentdeck workbench` inspect control. They remain read-only except for recording the chat turn and its audit event. They must not spawn, dispatch, capture, ack, release, advance the loop, call a provider, or read/write tmux.
- Chat ledger-mode responses must reuse the workbench ledger and lineage cards through `ledger_card` and `lineage_card`.
- Chat audit-mode responses must reuse the workbench audit card through `audit_card`, recommend `agentdeck events --limit 20`, include a `control_registry_card` filtered to the audit inspect control, list `control_registry_card` in `intent_card.secondary_embedded_cards`, and remain read-only except for recording the chat turn and its audit event.
...
- Chat policy-mode responses must reuse the workbench control mode projection through `control_mode_card`, include a `control_registry_card` filtered to `scope=policy` / `card=control_mode_card`, recommend an explicit `agentdeck policy set-mode --mode <mode>` command, and use action-specific next labels for ask, approval, and autonomous requests. The registry selection points at the target `kind=set_mode` control: enabled ask/approval controls may expose `selection.next_command` matching the response `next_command`, while disabled autonomous controls keep their blocker and must leave `selection.next_command=null`. Policy-mode must not mutate `.agentdeck/config.toml`.
