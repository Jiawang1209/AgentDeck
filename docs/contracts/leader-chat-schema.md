...
- Chat review-gate responses must reuse the workbench `review_gate_card`, recommend `agentdeck workbench`, include a `control_registry_card` filtered to `scope=review_gate` / `card=review_gate_card`, list `control_registry_card` in `intent_card.secondary_embedded_cards`, and remain read-only except for recording the chat turn and its audit event. They must not release, merge, ack inbox items, dispatch follow-up work, advance the loop, call a provider, or read/write tmux.
