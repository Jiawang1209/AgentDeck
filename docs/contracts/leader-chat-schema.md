...
When the human asks to send input to a single running agent, runtime-mode embeds `runtime_action_card` as the primary card. The card contains `action=send`, the target `agent_id`, role, runtime status, pane id, exact `command`, `preview_text`, `safety=explicit_runtime`, `requires_explicit_user=true`, and `controls[]` for inspecting the target terminal and explicitly sending the input. The same response also includes `runtime_card`, `terminal_session_card`, and a `control_registry_card` filtered to `runtime_action_card`, with `selection.selected_control` pointing at the send control whose command matches the top-level `next_command`. This card is an execution-before-confirmation affordance only: chat does not send tmux input, create plans, create approvals, create messages, create jobs, or mutate runtime state.
