...
When `continue_card` is present, `validate_leader_chat_contract()` reuses `validate_continue_contract()` and prefixes nested errors with `continue_card:`. When continue-mode recovery recommends a reply capture and `trace_card` is present, the validator rejects `intent_card.embedded_card` values other than `trace_card`.
