...
When continue-mode recovery points at a pending inbox item, the response also includes that agent's `inbox_card`; when recovery points at approvals, the response also includes `approval_card`; when more than one approval is already approved, the embedded `continue_card.next_command` may be promoted to `agentdeck approval dispatch-ready --confirm`; when recovery points at stale runtime, the response also includes `runtime_card`. Continue-mode remains read-only: embedded cards are display/action affordances, not automatic ack, approve, dispatch, refresh, spawn, stop, or tmux input.
