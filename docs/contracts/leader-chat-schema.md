...
The real card also includes Leader scheduling entries for `plan`, `review`, and `apply_action`, plus `policy`, `provider_switch`, `skill_import_preview`, and `skill_load_preview` entries for explicit configuration or skill lifecycle commands. `plan` must use `safety=plan_only` and its control command should point at the explicit `agentdeck leader plan --task <goal>` planning entrypoint; `review` and `apply_action` must use `safety=safe_apply`, with review pointing at `agentdeck leader review --plan-id <plan_id>` and apply_action pointing at `agentdeck leader apply-action --action-id <action_id>`; `policy` uses `safety=explicit_user` and points at `agentdeck policy set-mode --mode <mode>`; `provider_switch` uses `safety=explicit_user` and points at `agentdeck leader set-provider --provider <provider> --model <model>`; `skill_import_preview` and `skill_load_preview` use `safety=explicit_user` because the suggested next command changes skill registry or replayable context, even though the preview route itself is read-only. Read-only views such as `workbench`, `continue`, `leader_status`, `runtime`, `capture`, `role`, `ledger`, `trace`, `queue`, `approval`, `inbox`, and setup diagnostics use `safety=inspect` unless the downstream mode explicitly recommends an explicit runtime or provider configuration command. The `leader_status` capability points at `agentdeck leader status`, advertises `leader_status_card`, and lists `查看 Leader 状态`, `刷新 Leader 状态`, `Leader 概览`, `leader status`, `leader refresh`, and `leader overview` in `example_messages`, so GUI and natural-language shells can discover the same read-only status surface exposed by the direct route.
