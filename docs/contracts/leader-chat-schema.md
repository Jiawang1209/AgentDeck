...
The real card also includes Leader scheduling entries for `plan`, `review`, and `apply_action`, plus `policy`, `provider_switch`, `skill_import_preview`, `skill_load_preview`, and `learning_review` entries for explicit configuration, skill lifecycle, or learning-review commands. `plan` must use `safety=plan_only` and its control command should point at the explicit `agentdeck leader plan --task <goal>` planning entrypoint; `review` and `apply_action` must use `safety=safe_apply`, with review pointing at `agentdeck leader review --plan-id <plan_id>` and apply_action pointing at `agentdeck leader apply-action --action-id <action_id>`; `policy` uses `safety=explicit_user` and points at `agentdeck policy set-mode --mode <mode>`; `provider_switch` uses `safety=explicit_user` and points at `agentdeck leader set-provider --provider <provider> --model <model>`; `skill_import_preview` and `skill_load_preview` use `safety=explicit_user` because the suggested next command changes skill registry or replayable context, even though the preview route itself is read-only; `learning_review` uses `safety=inspect` because the route only displays the learning card and explicit suggestion commands. Read-only views such as `workbench`, `continue`, `leader_status`, `runtime`, `capture`, `role`, `ledger`, `trace`, `queue`, `approval`, `inbox`, and setup diagnostics use `safety=inspect` unless the downstream mode explicitly recommends an explicit runtime or provider configuration command. The `leader_status` capability points at `agentdeck leader status`, advertises `leader_status_card`, and lists `查看 Leader 状态`, `刷新 Leader 状态`, `Leader 概览`, `leader status`, `leader refresh`, and `leader overview` in `example_messages`, so GUI and natural-language shells can discover the same read-only status surface exposed by the direct route.

Every capability item includes `controls[]` using the same `kind`, `label`, `command`, `safety`, `enabled`, and `blocker` shape as intent controls; `agentdeck contract leader-chat` exposes the same shape as `capability_control_fields`. Direct read-only commands such as `agentdeck workbench` are enabled. Template commands may only use known placeholders: `<goal>`, `<plan_id>`, `<action_id>`, `<agent_id>`, `<mode>`, `<provider>`, `<model>`, `<SKILL.md>`, `<name>`, or `<purpose>`. `capability_placeholders[]` exposes that whitelist with the matching blocker for each placeholder so GUI clients can render template inputs without parsing this Markdown document. Template commands with those placeholders must be disabled and must include a matching blocker such as `requires goal text`, `requires plan_id`, `requires action_id`, `requires agent_id`, `requires control mode`, `requires leader provider`, `requires leader model`, `requires SKILL.md path`, `requires skill name`, or `requires purpose`. Capability controls must keep `command` and `safety` aligned with their parent capability item.
