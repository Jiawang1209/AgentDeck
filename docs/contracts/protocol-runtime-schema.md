...
The status response has exactly these fields, in order: `mode`, `contract_version`, `project`, `runtime_backend`, `agent_sessions`, `protocol_turns`, `transport_updates`, `permission_requests`, `protocol_state_transitions`, and `controls`. `mode` is `protocol_runtime_status`. `project` and `runtime_backend` are non-empty strings.
...
- `protocol_state_transitions`: `count`, `by_entity_type`, `items`; each compact item exposes only `transition_id`, `entity_type`, `entity_id`, `from_state`, `to_state`, optional compact `reason`, and `created_at`. Persisted `details` are deliberately absent.
...
The five summaries are independently bounded to their latest 20 records. Before any transition window is projected, `StateStore` builds strict session/turn/permission ID maps once and validates the complete append-only history in overall O(S+T+P+X): exact source rows, duplicate IDs, known entity types and identities, legal edges, and a continuous `from_state` chain from each immutable base record. Corruption outside the visible window therefore fails closed. Session, turn, and permission summaries replace only their compact current `state`/`status` with this derived value; base records are not rewritten. Existing state files with no transition collection render their original base states.

The response validator also cross-checks a complete transition window (`count <= 20`) against complete visible base summaries: every transition entity must exist, its type and ID prefix must agree, each chain starts at `created` (session/turn) or `pending` (permission), consecutive edges are continuous, and the final `to_state` equals the corresponding compact derived state/status. When a summary count exceeds 20, the validator does not require a parent omitted by the bounded window to be visible; visible rows still retain strict field, prefix, edge, ordering, and duplicate checks.

When a parent summary has `count <= 20`, it is complete: every turn must resolve its session item, and every update or permission must independently resolve both its session and turn items. When a parent summary has `count > 20`, a child may reference a valid parent outside that response window. Whenever a referenced turn item is visible, its session ID must match the child's session ID regardless of summary counts. Before ProjectView creates these bounded summaries, `StateStore` validates the complete source collections with linear ID maps.
...
Discovery exposes `schema_version`, `contract_version`, `status_command`, `contract_path`, `contract_exists`, response/summary/item/capability/control field lists, the session/turn/update/permission enums, transition entity types, the stable latest-window limit of 20, `transport_kinds`, `project_view_contract`, and `workbench_contract`. With `--example`, it additionally exposes `example_protocol_runtime` and same-source example field lists.
