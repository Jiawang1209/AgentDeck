...
- `selection` is derived from `filters.control_id` and `items[]`; `selection.blocker` only explains unmatched selection state, and `agentdeck controls` never uses it to execute or authorize commands.
