...
Each `turns[]` item contains exactly `step`, `agent_id`, `status`, and `handoff`. Steps are contiguous from one, the agent must be one of the frozen selected Workers, and the list cannot exceed `step_count`. A completed Mission must expose exactly `step_count` completed turns. A present handoff contains only the compact fields `step`, `agent_id`, `status`, `summary`, `verification`, `risks`, `next_steps`, `artifact_paths`, and `trace_command`; it must agree with its enclosing turn. `trace_command` must fully match `agentdeck trace --id rep_<12 lowercase hex>` with no suffix, newline, flag, or extra argument. Raw prompt, pane output, handoff token, message/job/reply ids, full output, other commands, credentials, and provider secrets are excluded.
