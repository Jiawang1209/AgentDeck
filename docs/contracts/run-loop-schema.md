...
- Dependency-satisfaction semantics (2026-08-03; replaces "earliest incomplete step only"): a wave dispatches every approved approval whose **inputs are ready**. A step counts as complete when its approval is rejected, or dispatched with a recorded reply — that definition is unchanged. Dependencies are **derived by the program, never authored by the Leader** (pure module `agentdeck.step_dag`, nothing persisted, no migration): an ordinary step depends on the step before it, every member of a review group depends on the step before the group's *first* member (so a group fans out in one wave), and a step that follows a group depends on *every* member of it. The guard consumes the transitive closure of that relation, which makes a plan without review groups **provably identical** to the old rule — "all of step N's ancestors are complete" is exactly "N is the earliest incomplete step" on a chain. Steps whose inputs are not ready stay `approved` and are reported in `skipped[]` with `reason="awaiting earlier step completion"`.
- Same-agent contention: parallelism's one new risk is two ready steps aimed at one pane. An agent already holding a dispatched-but-unreplied message receives nothing more this wave — same rule as `run-loop --all` (`_busy_agents`), and the busy set grows as this wave dispatches. The held approval stays `approved` and is reported in `skipped[]` with `reason="agent busy this wave"`, deliberately distinguishable from the ordering hold. Approvals are never merged: every step still passes its own approval gate, the allowlist and `max_approvals`/`max_review_rounds` budgets are unchanged, no agent is ever force-spawned, and `worktree merge-plan` still merges in step-number order.
- When a wave held approvals (for either reason), nothing actually blocked, and an earlier step still awaits its reply, the gate reports `waiting_for_reply` for that earlier step (with its explicit capture command) instead of recommending a dispatch of an intentionally held approval. Ingested replies are reported in the optional `captured_replies[]` field (`message_id`, `reply_id`, `agent_id`, `captured_from`, `trace_command`). When the reply file does not exist (or holds no structured reply), behavior is unchanged: run-loop stops at `waiting_for_reply`, hands the explicit `agentdeck capture-reply --agent <id> --message-id <id>` command to the human, and carries the optional read-only `reply_file_ready` field (`false` in that state). `RUN_LOOP_RESPONSE_FIELDS`, the required set, is unchanged; `captured_replies` and `reply_file_ready` are optional derived fields. This file-channel ingestion boundary was explicitly human-approved on 2026-07-25 after the file channel passed live verification with both codex and Claude Code workers.
