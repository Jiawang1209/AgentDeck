...
- `acceptance_criteria`: read-only display of the reviewed plan's G2 planner-brief acceptance criteria — `null` for single-stage plans, a list of non-empty strings for split plans. This is display/audit data only; the current review gate does not score against it, and it never authorizes dispatch.
- `verdict_summary`: read-only G5 quantified-review projection derived from the plan's latest reply that carried a valid `review-verdict/v1` payload, aligned with the plan's acceptance criteria. Exactly `criteria_total`, `passed`, `failed`, `unknown`, `overall`, `score` (nullable), `unverified[]` (plan criteria the verdict did not cover), `extra[]` (verdict criteria not in the plan), and `group`; `null` when no reply carries a verdict. It never changes the review gate, `next_action`, or any approval/dispatch behavior.
...
`verdict_summary.group` is the review-group provenance of that projection: exactly `size` (int ≥ 1, equal to the member count), `complete` (bool), `rule` (always `any_fail_blocks`), and `members[]` of exactly `agent_id`, `step`, `overall`, `reply_id`. A plan without a `[review].reviewers` group still projects an implicit `size=1` group whose single member is the reviewer whose reply supplied the verdict, so single- and multi-reviewer paths are structurally identical for GUI clients. With a configured group, `overall` is the any-fail-blocks aggregate of the members that reported. `complete` is `true` only once **every** member of the latest group replied with a parseable verdict; when a member's reply carries no valid verdict the group still projects with `complete: false`, that member's `overall` is `null`, and the aggregate covers the reporters — so a sibling's `fail` stays visible and `run-loop --merge-on-complete` withholds the automatic merge (the explicit human `worktree merge-plan --confirm` is never gated). A rework round is still only triggered by a *complete* group, so a partial group can never burn an iteration round. A group with no verdict at all projects `verdict_summary: null` as before. `group` is display/audit provenance only — it never authorizes dispatch, changes the review gate, or overrides approval semantics.
