...
- `agentdeck leader chat --message "帮助"` / `"help"` / `"你能做什么"` / `"命令面板"` 必须进入只读 `mode=help`，嵌入 `capability_card` 作为自然语言壳和未来 GUI 命令面板的能力发现入口；它不调用 provider、不创建 plan/action/approval/message/job/inbox、不读取 pane、不发送 tmux 输入。
