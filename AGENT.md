...
- 所有 `agentdeck leader chat` 响应都必须包含 `intent_card`，用于 GUI/自然语言壳解释 mode、matched_intent、route_source、embedded_card、read_only、next_command 和 requires_explicit_user；新增 chat mode 时必须同步 `LEADER_CHAT_INTENT_CARD_FIELDS`、validator、contract docs、README、HISTORY 和测试。
