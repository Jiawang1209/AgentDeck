...
- **Admission-before-completion durability**: transport admission returns a compact submitted receipt plus a bounded cold completion factory. The supervisor persists and rereads that receipt before creating any Worker completion awaitable, so a post-admission failure retains the submitted boundary for later recovery rather than appearing undispatched.
...
- **Review-hardened cold completion boundary**: `TransportExecution` now accepts only a cold completion factory, never a pre-started coroutine, Task, or Future. The factory is invoked only after an async submitted-receipt write completes and the authority callback rereads the exact submitted record. Persistence, admission, completion, and authority errors are converted to static sanitized `WorkerAttemptError` messages without exception chaining; a persistence failure never creates or starts completion work.
...
- **Native-cold factory and isolated callbacks**: completion admission accepts only a coroutine function and rejects direct, closure-captured, or default-captured Task/Future values; captured hot futures are cancelled before the event loop can run them. The post-persistence factory result must itself be a native coroutine—Tasks, Futures, and generic awaitables are cancelled/drained or rejected. Authority, persistence, and executor callbacks receive independent deep copies, while transport choice and lineage remain bound to the untouched canonical baseline.
- **Bounded claim lifecycle and clean errors**: a pre-admission executor failure releases its in-process claim so the still-prepared durable attempt may retry. Confirmed submitted persistence clears the transient claim and relies on durable state; unknown persistence moves to a recovery fence which blocks retry until authoritative state changes, then self-clears. Concurrent identical dispatches call one transport. External callback exceptions are captured and discarded before a static error is raised, leaving both `__cause__` and `__context__` empty.
- **One canonical handoff record**: `CanonicalHandoff` is now the only validated compact record and includes the exact token, approved reply fields, artifact path/hash pairs, and trace ids. Both daemon supervision and legacy workflow construction call `build_canonical_handoff()`; the historic workflow dictionary is produced only by an explicit `render_legacy_handoff()` compatibility projection and has no independent reply validation or field source.
- **TDD evidence**: initial RED failed collection because `agentdeck.daemon.supervisor` did not exist. First review RED produced 23 failures covering hot completion, persistence leakage, incomplete authority, duplicate dispatch, forged ownership, loose types, missing/mismatched tokens, and mutable outcome aliasing. Second review RED exposed hidden hot Tasks, callback aliasing, stuck claims, sensitive exception context, and dual handoff formats. GREEN is 37 passed for the focused supervisor suite, 49 passed with legacy workflow, and 248 passed with one skip for supervisor, Task 7 Mission attempt, Mission orchestration, conversation transport, ACP, tmux, and workflow regression; compileall and `git diff --check` pass.
