...
- **Crash and product acceptance**: added a nine-boundary real-SIGKILL recovery matrix with zero duplicate dispatches, plus one disposable real-daemon acceptance that disconnects its first client, reaches a permission pause, renders recovery through a new bare `agentdeck` PTY, confirms the exact decision, and completes ordered ACP then tmux Workers only after the predecessor compact handoff is durable.
- **Transcript-free daemon ACP ledger**: a genuine acceptance RED showed daemon Mission `transport_updates` retained full streamed text. The daemon sink now persists only the canonical full-payload hash and byte count while retaining bounded raw fragments only in process memory for canonical handoff parsing. Replies, handoffs, ProjectView, contracts, events, snapshot hashes, and file effects remain mutually consistent without private-reasoning/full-transcript/secret markers.
- **Evidence and honest live boundary**: focused acceptance/crash tests pass twice (`10 passed` each), acceptance/crash/recovery passes `117`, daemon tests pass `889`, and the full suite passes `2757` with one skip. Existing Claude Agent ACP `0.58.1` real foreground acceptance passes. The full real M2 rehearsal remains explicitly blocked: installed Codex/Claude CLI Leader outputs did not reliably satisfy the strict Mission JSON contract, and a deterministic-plan transport-only run reached real Claude ACP success plus durable handoff before real Codex tmux remained submitted without a correlated reply beyond the bounded window. No install/auth changes, residual process, or temporary project remained.
