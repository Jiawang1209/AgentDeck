...
- **Compact generation provenance normalization**: added one provider-neutral builder for the provider/model, constraint mode, canonical schema identity, bounded attempt facts, and frozen selected-Worker/count authority. It rejects invalid modes, unrelated schemas, malformed attempt counts, and unsafe provider identities with fixed credential-free errors; it never derives authority from provider plan output and does not grant authorization.
...
- **TDD and compatibility evidence**: the first schema test run failed at collection with the expected missing `agentdeck.providers.plan_schema`; construction/authority tests then passed, semantic validation tests drove the validator extraction, and the provenance slice first failed on its missing public builder. An independent regression recomputes the exact UTF-8 JSON canonical encoding before hashing so deterministic but noncanonical implementations cannot pass. The focused schema suite passes 39 tests while the legacy OpenAI-compatible provider suite passes 31 tests.
