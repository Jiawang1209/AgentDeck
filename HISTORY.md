...
- **Verification**: the final focused reconnect/migration/conversation group passes 55 tests, the required daemon/conversation/contracts/agent-CLI/dashboard regression passes 1,829 tests, and the full suite passes 2,724 tests with one skip. `python -m compileall -q src tests`, `git diff --check`, and the no-temporary-daemon/ACP-worker process audit pass.
