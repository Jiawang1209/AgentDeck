...
- **Bounded multi-client observation**: each connection owns bounded request/event queues and one serialized writer. Multiple observers can subscribe concurrently; event publication never awaits a subscriber, and a full subscriber queue closes only that connection. Frames, initial handshake/status waits, ordinary request waits, startup retry, and event waits are bounded, while timed-out request ids are retained only in a small tombstone window so late replies cannot poison a still-healthy connection.
...
- **Exact endpoint and process safety**: the server manually pre-binds the project-local socket and gives asyncio the already-bound descriptor, avoiding asyncio's path-based stale-socket unlink race. Shutdown removes only the exact socket device/inode created by that server. On-demand startup uses `sys.executable` plus a fixed argv list, no shell parsing, a detached session, project-local mode-0600 stdout/stderr files opened component-by-component without following symlinks, bounded retry, and cancellation that never signals an unverified process.
...
- **TDD evidence**: the initial RED was the missing client/server modules; follow-up REDs covered explicit request encoding, stalled-request timeout, external log symlinks, stale lease diagnostics, and path-delegated socket replacement. The real IPC suite includes a subprocess fixture that acquires Task 3 ownership, serves a verified endpoint, reconnects, and cleans up. It is 18 passed under `PYTHONASYNCIODEBUG=1` with warnings as errors and no pending-task, transport, callback, or resource warning; the combined protocol/process/lease/IPC regression is 191 passed, with compileall and `git diff --check` passing.
