...
- **Immutable controller state machine**: added frozen observer, controller lease, takeover preview, audit event, and transition records covering first grant, renewal, expiry, explicit release, and preview-bound takeover. Privileged transitions and audit events carry private builder capabilities, so public hand construction or `dataclasses.replace` cannot bypass release credentials or exact takeover confirmation; audit payloads are recursively detached and frozen. Observer registration never grants mutation authority, while each new controller generation increases monotonically.
...
- **Atomic compact persistence**: StateStore revalidates transitions against the persisted controller lease while holding the existing protocol mutation lock, then atomically commits only the six-field lease summary and compact audit event to `daemon_event_outbox`. A shared strict EventRecord validator makes both preflight and locked replay validate exact fields, repository-compatible safe event identities, lowercase event types, aware timestamps, recursively finite JSON payloads, and unique IDs in both the outbox and journal; malformed JSON or structured corruption fails closed. Expiry uses a deterministic lease-id/generation event identity, so a repeated expiry is rejected from either the durable outbox or the event journal across restart without expanding the lease schema. Invalid, stale, malformed, duplicate, or backward transitions are zero-write; legacy state receives the additive fields on first valid commit.
- **TDD evidence**: the initial RED was the missing lease module; follow-up REDs covered audit action shape, lock-file zero-write, exact preview objects and timestamps, TTL conversion/offset overflow, forged backward/expired renewal and release, incoherent first-grant timestamps, hand-constructed privileged transitions, mutable audit payloads, malformed/duplicate outbox facts, restart-safe repeated expiry, malformed/duplicate journal records, legacy event identity compatibility, non-ASCII secret diagnostics, and malformed persisted leases. The focused suite is 49 passed, the combined lease/conversation/protocol/daemon regression is 353 passed, and compileall plus `git diff --check` pass.
