...
- **TDD evidence**: the initial RED failed collection because `agentdeck.daemon.scheduler` did not exist. Follow-up REDs exposed terminal Mission re-effecting, missing Worker results, unready Workers, active-attempt completion conflicts, incoherent reply/handoff lineage, drift from existing Mission enums, unconfirmed snapshot handling, nondeterministic fact-summary ordering, and under-validated decisions. Spec-review RED added eleven failures for premature completion and nonterminal reply/handoff advancement. GREEN is 73 passed for the scheduler suite and 433 passed for the required scheduler/autonomy/agent CLI regression; compileall and `git diff --check` pass.
