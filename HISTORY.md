...
- **TDD and regression evidence**: Mission RED was four expected failures for random logical identity and absent controller-lineage authority; focused GREEN was four passes plus a real detached-daemon preview/confirm pass. Shutdown RED was five expected failures for dropped cleanup, unshared close failure, and cancellation at all three admission stages; focused GREEN was five passes. Post-review RED additionally covered each force-stop post-commit failure stage, cleanup-queue forgery while OPEN, cancellation-swallowing ACP close, close-side `CancelledError`, prompt cancellation, and the activate-persist/return-cancel race. The final focused daemon service/transport/recovery/CLI group passes 195 tests; daemon/ACP/protocol regression passes 981 tests with one skip; the full suite passes 2,651 tests with one skip. `python -m compileall -q src` and `git diff --check` pass.
