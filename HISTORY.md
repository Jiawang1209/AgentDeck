...
- **Frozen Worker runtime identity**: execution snapshots now bind each Worker to a compact `runtime_identity_hash` over its exact configured command, ACP transport argv, and role prompt. Raw invocation and instruction text remain excluded from durable authority, while confirmation and later execution can fail closed when executable or instruction configuration drifts.
