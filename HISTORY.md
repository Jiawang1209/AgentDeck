...
- **Capability and path hardening**: endpoint cleanup now requires the exact immutable owner to retain a live lock descriptor for the same lock inode while an independent descriptor confirms exclusivity; forged, follower, mutated/replaced, or released ownership cannot delete endpoint files. Project runtime directories are opened component-by-component without following symlinks, endpoint symlinks fail closed before lock/metadata writes, and cleanup never follows or deletes an external target.
...
- **TDD evidence**: the initial RED was the missing process-lifecycle APIs; follow-up REDs covered recovery-decision/atomic-write stop facts, forged cleanup capability, runtime/endpoint symlinks, malformed wait bounds, and oversized polling intervals. The hardened real-process suite passes in five consecutive runs (46 passed each), the combined daemon lifecycle/protocol/process regression is 145 passed, and compileall plus `git diff --check` pass.
