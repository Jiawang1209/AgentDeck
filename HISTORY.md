...
- **Native Claude envelope**: `ClaudeCliProvider` now invokes exactly one `claude --print --output-format json --permission-mode plan` subprocess with the exact compact canonical request schema supplied through `--json-schema`. Its prompt requests only `goal`, `summary`, and `steps`; AgentDeck extracts only a strict successful `structured_output` object, rejects malformed/duplicate/non-finite/oversized/noncanonical envelopes and plans, rebuilds one-attempt native provenance locally, and retains the dict-returning public boundary. Official result, cost, duration, and session metadata are ignored and never persisted. No retry, provider fallback, dispatch, tmux access, or authority grant was added.
- **Private bounded stdout**: Claude native stdout is redirected to an AgentDeck-created exclusive `0o600` regular file inside a verified `0o700` temporary directory, while stderr goes directly to `DEVNULL`. The shared no-follow, owner/single-link/path-identity, stable-metadata reader bounds output at 2 MiB. Success, timeout, nonzero exit, OS/file/parse/schema failure, and close handling preserve fixed redacted stages and clean temporary output without retaining raw command, output, metadata, prompt, or path values.
- **Read-only capability preflight**: `cli_native_schema_ready()` runs one bounded five-second help probe for known Codex/Claude CLI mappings, redirects combined help output to the same private bounded-file boundary, requires the provider-specific native flags, and returns only fixed unsupported/executable-unavailable/capability-unavailable blockers. `LeaderGateway.describe()` keeps the executable check first, injects the probe for deterministic tests, advertises `native_json_schema` only after success, constructs no provider during preflight, and blocks generation without fallback when the probe fails.
