...
- **Closed Gateway edge provenance**: when an already validated Leader result is followed by cancellation, JSON serialization failure, or the final 2 MiB size gate, Gateway rebuilds the terminal error from that result's validated generation facts. A local attempt-one or native attempt-two result therefore retains its exact `attempt_count` and `constraint_mode` without retaining the plan value, raw output, serialization object, path, or retryability. Frozen authority supplied to `generate_mission` is now purely validated before backend description, executable/native-capability probes, provider construction, or planning; partial, unknown, duplicate, boolean, and out-of-range authority fails at the fixed prompt-only attempt-zero boundary, while the legacy both-`None` request remains valid.
...
- **TDD evidence**: Gateway RED produced 11 failures for the missing candidate field and four-field diagnostics; durable session RED produced four failures for absent response/event facts and authority classification; landing RED produced 13 failures for missing validation, storage, and direct provenance. Spec review added 13 Gateway RED failures for lost post-generation attempt facts and authority reaching executable probe/provider construction, while the stale strict fixtures produced two acceptance/Mission RED failures before exact provenance migration. Expanded regressions cover local, JSON-object, prompt-only, native-schema, and ACP success provenance; cancel/serialization/oversize after validated local/native results; exact constructor rejection; missing/extra/spoofed/wrong-type authority/schema/attempt/regeneration facts; zero backend/domain effects on invalid authority; deep-copy behavior; legacy compatibility; and hash stability.
