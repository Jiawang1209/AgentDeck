...
- **TDD evidence**: initial RED failed collection because `agentdeck.daemon.supervisor` did not exist. Review RED then produced 23 failures covering hot completion, persistence leakage, incomplete authority, duplicate dispatch, forged ownership, loose types, missing/mismatched tokens, and mutable outcome aliasing. GREEN is 27 passed for the focused supervisor suite and 183 passed with one skip for supervisor, Task 7 Mission attempt, conversation transport, ACP, tmux, and workflow regression; compileall and `git diff --check` pass.
