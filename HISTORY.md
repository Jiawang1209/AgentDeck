...
- **Anchored event journal and mutation lock**: every `events.jsonl` read and append now opens the journal relative to the already verified project `state` directory descriptor with `O_NOFOLLOW`, requires a regular file, and revalidates the opened inode against the named path. Append is a bounded 64 MiB copy-on-replace transaction: it reads the anchored journal, writes and fsyncs a project-local exclusive temporary regular file, proves the original path identity is unchanged, atomically installs the complete old-plus-new journal, and fsyncs the state directory. A journal inode moved after proof therefore never receives new bytes; generic event writes and daemon, conversation, and protocol outbox drains retain pending records on failure and retry idempotently. Every new authoritative writer first flocks the anchored state-directory inode, then takes the legacy `protocol-mutation.lock` flock in a fixed order for mixed-version compatibility, proves the named lock still matches its held regular-file descriptor, and revalidates the project anchor. Replacing the lock filename after proof cannot split current writers into independent lock domains or lose either state update.
