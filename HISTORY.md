...
- **TDD evidence**: initial RED failed collection because `agentdeck.daemon.supervisor` did not exist. Successive review REDs covered unsafe completion injection, persistence leakage, incomplete authority, duplicate dispatch, forged ownership, loose types, missing/mismatched tokens, mutable aliasing, stuck claims, sensitive exception context, dual handoff formats, process-local ambiguity, and mismatched receipt lineage across a real StateStore restart. The structural two-phase RED produced 33 failures. GREEN is 42 passed for the focused supervisor suite, 54 passed with legacy workflow, and 253 passed with one skip for supervisor, Task 7 Mission attempt, Mission orchestration, conversation transport, ACP, tmux, and workflow regression; compileall and `git diff --check` pass.
