...
- **TDD and regression evidence**: Mission RED was four expected failures for random logical identity and absent controller-lineage authority; focused GREEN was four passes plus a real detached-daemon preview/confirm pass. Shutdown RED was five expected failures for dropped cleanup, unshared close failure, and cancellation at all three admission stages; focused GREEN was five passes. Final quality-review REDs covered mixed predecessor terminal evidence, swallowed outer cancellation, silently successful disconnect failure, cancellation of the durable cleanup Future, duplicate-native activation identity, unbounded stubborn cleanup, unbounded service close, poisoned admission reservation, force-stop RPC misreporting, and a truncate/write outbox drain observed twice by the real background Mission acceptance. The final focused governance/service/transport/recovery/CLI group passes 238 tests; daemon/ACP/protocol regression passes 991 tests with one skip; the full suite passes 2,661 tests with one skip. `python -m compileall -q src` and `git diff --check` pass.
