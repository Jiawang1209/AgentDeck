...
- **Contract and TDD evidence**: the Leader backend contract now states that native JSON Schema is a generation capability only—not Worker readiness, tmux identity, permission, dispatch/execution authority, or fallback. The initial Claude test failed because the legacy path had no private stdout sink and still used text output; the initial probe and Gateway tests failed on the missing function/injection boundary. Focused coverage now includes exact command/schema/model placement, envelope and semantic validation, strict decoder limits, ignored metadata redaction, bounded private output and cleanup, fixed subprocess errors, no fenced-parser use, direct dict/Orchestrator compatibility, supported/missing-flag/timeout/nonzero/missing-executable/unknown probes, and blocked-provider construction. The provider compatibility suite now supplies Claude's strict native envelope through the private stdout descriptor and expects native provenance/stages, while generic `CliLeaderProvider` tests retain the legacy fenced-parser coverage for non-native subclasses.
