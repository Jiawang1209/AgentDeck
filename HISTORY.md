...
- **TDD evidence**: the initial RED was the missing process-lifecycle APIs; follow-up REDs covered recovery-decision/atomic-write stop facts, forged cleanup capability, runtime/endpoint symlinks, post-validation runtime swaps, descriptor failure cleanup, malformed wait bounds, and oversized polling intervals. The hardened real-process suite passes in five consecutive runs (51 passed each), the combined daemon lifecycle/protocol/process regression is 150 passed, and compileall plus `git diff --check` pass.
