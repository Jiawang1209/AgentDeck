...
- **Crash and product acceptance**: added a nine-boundary real-SIGKILL recovery matrix with zero duplicate external admissions, plus one disposable real-daemon acceptance that creates and confirms a Mission through a bare `agentdeck` PTY, disconnects after the confirmation transaction is durably consumed, reaches a permission pause, renders recovery through a fresh bare PTY, confirms the exact decision, and completes ordered ACP then tmux Workers only after the predecessor compact handoff is durable. Pending ACP permission after daemon loss remains conservatively `ambiguous`, not replayable `waiting_human`; force-stop leaves the Mission `interrupted` while preserving the active attempt's unknown effect as `ambiguous`.
