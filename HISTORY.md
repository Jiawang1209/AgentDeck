...
- **Mixed-version lock replacement CAS/rollback**: every state and journal effect now revalidates the held legacy lock descriptor against the current `protocol-mutation.lock` name before and after I/O. Atomic state saves additionally compare the exact pre-replace state inode/content, retain that displaced descriptor through commit, and on authority drift roll back only while the canonical name still identifies AgentDeck's exact installed bytes on a different inode. Rollback first acquires the replacement filename lock, so a cooperating older writer finishes before recovery; an in-place older write is restored from the retained inode, while an older writer that already atomically replaced canonical state/journal is preserved and the current mutation fail-stops. Journal rollback follows the same bounded 64 MiB rule and never clears a pending outbox after drift. Six deterministic mixed-version races cover post-proof, post-CAS/pre-replace, post-effect, replacement-lock contention, and journal before/post replacement; the complete daemon suite passes `963` tests after this closure.
