...
- **TDD and regression evidence**: Mission RED was four expected failures for random logical identity and absent controller-lineage authority; focused GREEN was four passes plus a real detached-daemon preview/confirm pass. Shutdown RED was five expected failures for dropped cleanup, unshared close failure, and cancellation at all three admission stages; focused GREEN was five passes. Final quality-review REDs covered mixed predecessor terminal evidence, swallowed outer cancellation, silently successful disconnect failure, cancellation of the durable cleanup Future, duplicate-native activation identity, unbounded stubborn cleanup, unbounded service close, poisoned admission reservation, force-stop RPC misreporting, a truncate/write outbox drain observed twice by the real background Mission acceptance, admission/prompt sessions left busy by repeated outer cancellation, close-error endpoint leakage, and cancellation-resistant event-loop teardown. The final focused governance/service/transport/recovery/CLI group passes 242 tests; the broad test-name-selected daemon/ACP/protocol regression passes 1,218 tests with one skip; the full suite passes 2,665 tests with one skip. `python -m compileall -q src` and `git diff --check` pass.
