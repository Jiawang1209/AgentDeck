...
- **Strict additive `project-view/v1` contract**: discovery now publishes the ordered nested field list, examples include valid provenance, and ProjectView/trace validators reject missing/extra fields, unknown modes, malformed or uppercase hashes, boolean/out-of-range attempts, regeneration mismatches, duplicate/unknown Worker ids, step/provider/model mismatches, and forbidden nested semantic keys. The metadata is audit provenance only—not readiness, pane identity, permission, dispatch authority, or approval bypass.
