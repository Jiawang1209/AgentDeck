...
- 同步 README、`docs/contracts/leader-chat-schema.md`、CLAUDE.md 和 AGENT.md，明确 continue-mode 可在 reply recovery 下嵌入 `trace_card`，`intent_card.embedded_card` 应优先指向 `trace_card`，并必须复用 `validate_trace_contract()`。
- 验证记录：已先确认红测失败，`agentdeck leader chat --message "继续"` 在 `reply_waiting` 时最初返回 `trace_card=None`；随后补充红测确认 `intent_card.embedded_card` 最初仍是 `continue_card`；contract 红测确认 `validate_leader_chat_contract()` 最初会放过 `reply_waiting` 下错误的 `embedded_card=continue_card`；本轮新增红测确认 validator 最初也会放过错误的 inspect command；实现后目标测试 `conda run -n agentdeck pytest tests/test_contracts.py::test_validate_leader_chat_contract_requires_reply_waiting_trace_intent_card tests/test_contracts.py::test_validate_leader_chat_contract_requires_reply_waiting_trace_inspect_control tests/test_leader_cli.py::test_leader_chat_continue_embeds_trace_card_for_reply_waiting -q` 通过；相关回归 `conda run -n agentdeck pytest tests/test_contracts.py::test_validate_leader_chat_contract_requires_reply_waiting_trace_intent_card tests/test_contracts.py::test_validate_leader_chat_contract_requires_reply_waiting_trace_inspect_control tests/test_leader_cli.py::test_leader_chat_continue_embeds_trace_card_for_reply_waiting tests/test_contracts.py::test_leader_chat_contract_response_includes_example_without_drift tests/test_contracts.py::test_validate_leader_chat_contract_accepts_example tests/test_contracts.py::test_validate_leader_chat_contract_requires_inspect_control_safety tests/test_contracts.py::test_validate_leader_chat_contract_requires_intent_next_command_match -q` 7 项通过；`conda run -n agentdeck python -m compileall src tests`、`git diff --check` 和 `conda run -n agentdeck pytest -q` 通过，全量测试 296 项通过。
