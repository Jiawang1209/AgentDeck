...
- **Bounded result-file authority**: Codex success consumes only the named result file, never stdout or fenced JSON. The reader uses no-follow/nonblocking descriptor opens, requires a regular file, rejects stat-visible and read-race output above 2 MiB, decodes strict UTF-8, parses exactly one JSON object, and validates it against resolved request authority. Missing, symlink, directory, FIFO, malformed, non-object, oversized, nonzero, timeout, and semantic schema failures preserve the existing credential-free stages; parse failures use `invalid_output_envelope` and semantic failures retain only the fixed provider diagnostic code.
- **Cleanup and redaction**: schema/result files live under `TemporaryDirectory(prefix="agentdeck-leader-")` and are removed after success, timeout, nonzero exit, and every file/parse/schema failure. Captured stdout/stderr, prompt, argv, executable and temporary paths are discarded and never enter result, error, or durable state. No shell invocation, retry, provider fallback, tmux access, dispatch, or approval mutation was added.
- **TDD and compatibility evidence**: the first exact Codex test failed with `AttributeError: 'CodexCliProvider' object has no attribute 'plan_result'`, proving the native path and flags were absent before implementation. New coverage exercises secure schema creation, exact command/model placement, cleanup, stdout no-fallback behavior, unsafe/special files without blocking, both oversize defenses, malformed envelopes, redacted process failures, exact semantic diagnostic codes, dict compatibility, and Orchestrator provenance reconstruction. Legacy fenced-stdout parser coverage now explicitly exercises Claude. The focused four-suite regression passes 90 tests.
