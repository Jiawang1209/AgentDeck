...
- **Disconnect-safe background lifetime**: service progress is independent of any client connection. A bounded real subprocess test closes the client-side channel, then proves the service validates the planner result before activating the reviewer and exits cleanly with deterministic timeout/termination cleanup. The sequence is mediated by AgentDeck rather than direct Worker-to-Worker observation.
...
- **TDD evidence**: initial RED failed because `agentdeck.daemon.service` did not exist. Follow-up REDs covered reconcile-before-server ordering, failed-start zero effect, one effect per iteration, owned-loop shutdown, service-queue serialization, Worker completion isolation, exact digest drift, immutable RPC JSON admission, daemon-unavailable recovery, and real subprocess disconnect continuation. A broad regression exposed an empty-outbox flush race with an external StateStore update; the service now performs steady-state flushes only when a fresh read proves pending events, while startup remains strictly full outbox-first. GREEN is 10 passed for the new service/background/CLI slice, 1,386 passed for the broad daemon/conversation/Mission/workflow/CLI regression, and 2,534 passed with one skip for the full suite; compileall and `git diff --check` pass.
