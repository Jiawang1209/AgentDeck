...
- **Task 7 authority and idempotency**: removed the three-field shadow attempt model. Supervision now validates the exact durable 13-field Mission attempt record through the StateStore validator, requires `state=prepared`, compares Mission/step/Worker/transport/dispatch-key/snapshot/timestamp authority with a fresh persistence callback, and rejects an already submitted/running or in-process claimed dispatch key before any second transport call. Route ownership independently must be exactly `agentdeck_owned`, and boolean/container/item types are strict.
...
- **Isolated callbacks and durable ambiguity**: authority, persistence, admission, and completion callbacks receive independent deep copies, while transport choice and lineage remain bound to the untouched canonical baseline. A pre-admission failure releases its transient claim; successful submitted persistence clears it and relies on durable state. Unknown persistence calls the minimal StateStore ambiguity transition, which accepts prepared/submitted, records `receipt_persistence_unknown`, appends `mission_attempt_ambiguous`, and makes a fresh supervisor reject the non-prepared attempt without another admission. A failed ambiguity write remains fail-closed in the current supervisor.
- **Canonical ambiguity transition key**: a mismatched admission receipt can no longer become the lookup authority for its own ambiguity transition. StateStore locates and updates the attempt only with the frozen baseline `dispatch_key`; the validated observed receipt key is retained solely as `observed_dispatch_key` audit evidence beside `expected_dispatch_key`. If that ambiguity write cannot be proven durable, the supervisor returns the stable `Worker ambiguity persistence failed` error and retains its in-process claim.
- **Daemon-wide durable admission claim**: added the atomic `prepared -> admitting -> submitted` StateStore boundary before external ACP/tmux admission. Concurrent supervisors sharing a project store can no longer both admit the same dispatch: only the winner of `mission_attempt_admission_claimed`, verified by an exact authority reread, reaches the transport. Submitted receipt persistence cannot bypass `admitting`. A definite pre-admission failure atomically emits `mission_attempt_admission_released` and restores `prepared`; an unknown outcome leaves the explicit `admitting` fact for Task 10 recovery without adding a reconcile loop here. State and Task 8 scheduler enums treat `admitting` as active and await its resolution.
- **Submitted completion authority and global lineage**: completion callbacks now receive the exact durable `submitted` attempt reread after receipt persistence, while route and handoff lineage remain compared with the frozen prepared baseline. Admission claim/release and submitted/ambiguity receipt transitions validate dispatch-key uniqueness across the complete attempt history before any write, so corrupted duplicate lineage fails with zero state or event mutation.
