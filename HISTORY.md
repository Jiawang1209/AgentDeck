...
- **Verification**: the final focused reconnect/migration/conversation group passes 50 tests, the required daemon/conversation/contracts/agent-CLI/dashboard regression passes 1,824 tests, and the full suite passes 2,719 tests with one skip. `python -m compileall -q src tests`, `git diff --check`, and the no-temporary-daemon/ACP-worker process audit pass.
