...
- 加强 `validate_leader_chat_contract()`：当 continue-mode recovery 推荐 reply capture 且响应带 `trace_card` 时，validator 会拒绝 `intent_card.embedded_card` 不是 `trace_card` 的响应，避免 GUI 主卡语义漂移。
