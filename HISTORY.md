...
- **Safe daemon CLI**: added `daemon status/start/stop/logs` and hidden `_daemon serve`. Status checks endpoint presence without creating runtime paths and never starts the daemon; start reuses Task 5's verified `connect_or_start`; stop requires `--confirm`, a compatible verified handshake, the Task 3 keepalive gate, and a held metadata instance recheck before signaling; logs use bounded nofollow descriptor traversal and redact common inline secrets. The child installs bounded stdio, acquires exact ownership, serves bounded RPC, records compact ready/stopped lifecycle state, exits after idle grace or signal, and cleans only its verified endpoint.
