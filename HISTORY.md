...
- **TDD evidence**: the governance tests first failed on the absent module, queue execution guard, permission projection, durable preview APIs, forged preview fields, terminal permission replay, unsafe active-attempt pause, absent production Mission/permission/ownership/reroute/force-stop handlers, the missing live ACP permission waiter, transport-specific ownership projection, permissive reroute-record consumption, the split permission/binding save window, approval bypassing the real effect gate, weak ACP binding lineage, restart permission precedence, and synthetic return reconciliation. The final focused recovery/governance/transport/real-background regression passes 134 tests; the full suite passes 2,619 tests with one skip. Compileall and `git diff --check` pass.
