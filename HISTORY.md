...
- **Deterministic same-source recovery card**: ProjectView now exposes a compact `mission_recovery` card with reconciliation class, bounded progress/step facts, the latest three validated result hashes, active step/wait reason, safe decision affordances, trace commands, and workbench entry. Conversation reconnect and workbench reuse that exact ProjectView object. Rendering performs no provider call, tmux I/O, scheduling mutation, or transcript reconstruction; malformed recovery/attempt/reply records fail closed without leaking raw content.
- **Exact additive migration**: `agentdeck project migration-preview` is zero-write and binds the exact `state.json` byte hash, additive changes, legacy inspect-only Mission list, expiry, digest, consume-once identity, backup path, and confirmation command. `agentdeck project migrate ... --confirm` rejects drift, replay, expiry, wrong digest, overwrite, backup failure, and save failure without partial migration. It writes a sanitized project-local record of the affected additive paths' prior absence before atomic state replacement; Mission content, runtime credentials, and external files are excluded.
...
- **Verification**: the final focused reconnect/migration/conversation group passes 21 tests, the required daemon/conversation/contracts/agent-CLI/dashboard regression passes 1,800 tests, and the full suite passes 2,695 tests with one skip. `python -m compileall -q src tests`, `git diff --check`, and the no-temporary-daemon/ACP-worker process audit pass.
