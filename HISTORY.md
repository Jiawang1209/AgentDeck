...
- **ACP combined completion fence**: ACP native session creation is no longer persisted as a standalone `submitted` receipt while the same process still owes the prompt result. During ACP prompt I/O the durable attempt remains `admitting`; once both the native receipt and validated result exist, one StateStore method performs one protocol lock and one `_atomic_save` for submitted lineage, terminal result, and received reply evidence. A daemon crash or shutdown in flight therefore leaves an admission ambiguity for recovery rather than a false submitted state waiting for a result that cannot arrive. Exact duplicate completion remains read-only idempotent after the same reply advances from `received` to `validated`; an `invalid` reply, stale claim, conflicting retry, or invalid authority remains a zero-write conflict, while failed results atomically persist terminal failure without manufacturing reply evidence. tmux keeps its real two-phase submitted-receipt fence because its visible external effect remains independently observable after process loss.
