...
- **TDD evidence**: initial RED failed collection because `agentdeck.daemon.recovery` did not exist. Follow-up REDs covered persisted Mission/attempt drift, forged validated/handoff facts, terminal and pending-confirmation precedence, illegal decision transitions, orphan/cross-Mission references, multiple active attempts, multi-Mission corruption, state drift between read and commit, forged direct persistence, confirmation-before-first-attempt recovery, unaudited reply evidence, controlled permission status drift, protocol-transition CAS drift with an unchanged recovery outcome, malformed/orphan/broken protocol transition histories, corrupt retry bindings, complete protocol-turn CAS drift, noncanonical turn records, exact Session/Permission records, invalid pending-decision combinations, and retry-time unaudited reply corruption. GREEN is 454 passed for recovery/snapshot/scheduler/supervisor/protocol/conversation regression and 2,509 passed with one skip for the full suite; compileall and `git diff --check` pass.
