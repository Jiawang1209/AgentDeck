...
- **TDD evidence**: initial RED failed collection because `agentdeck.daemon.recovery` did not exist. Follow-up REDs covered persisted Mission/attempt drift, forged validated/handoff facts, terminal and pending-confirmation precedence, illegal decision transitions, orphan handoffs, multiple active attempts, multi-Mission corruption, state drift between read and commit, forged direct persistence, and the no-active-Mission startup case. GREEN is 45 passed for recovery, 428 passed for Task 7–10 plus required state/protocol regression, and 523 passed with one skip for the expanded snapshot/scheduler/supervisor/ACP/tmux/workflow regression; compileall and `git diff --check` pass.
