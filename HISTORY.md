...
- **TDD evidence**: RED first exposed the missing retry-limit preview API, duplicate Mission attempt identities, dropped declared-test/acceptance hashes, four accepted prior-attempt authority drifts, and an accepted existing-existing duplicate dispatch key. GREEN is 49 passed for the focused snapshot/attempt suite, 128 passed for the required Mission/conversation/workflow regression, 631 passed for Mission/contracts/dispatch/conversation-state, and 6 passed for targeted trace regression; compileall and `git diff --check` pass.
