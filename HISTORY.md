...
- **Lease-gated server-owned stop**: `daemon stop` now requires `--confirm`, the exact controller lease id, and generation, then sends `daemon.stop` over a compatible verified RPC connection. The daemon reloads and validates the persisted lease, rechecks held endpoint metadata against its ownership and durable lifecycle record, excludes only the requesting connection, applies the shared keepalive gate, and owns the shutdown event. The client no longer calls `os.kill`; missing, stale, expired, identity-drifted, concurrent-client, or active-work requests cannot stop the daemon.
- **Acknowledged shutdown and strict contracts**: shutdown is triggered by a server response-sent hook only after the accepted RPC response has completed `writer.drain()`, replacing timing-based delay. Daemon controls now require exact typed fields; client-session contracts reject incompatible/write, observer/write-or-lease, controller-without-lease, and none-with-identity combinations. Stop controls display the complete lease-gated command and remain disabled until current lease facts are supplied explicitly.
