...
- **Evidence and honest live boundary**: the crash/acceptance focused group passes twice (`12 passed` each), acceptance/crash/recovery passes `127`, daemon tests pass `899`, and the full suite passes `2778` with one skip. Existing Claude Agent ACP `0.58.1` real foreground acceptance passes. The full real M2 rehearsal remains explicitly blocked: installed Codex/Claude CLI Leader outputs did not reliably satisfy the strict Mission JSON contract, and a deterministic-plan transport-only run reached real Claude ACP success plus durable handoff before real Codex tmux remained submitted without a correlated reply beyond the bounded window. No install/auth changes, residual process, or temporary project remained.
- **Product docs and handoff**: English/Chinese README, both roadmaps, current handoff, and the sanitized validation report now describe disconnect-safe project-local M2, deterministic reconnect/migration, exact safety pauses, and future A2A/remote/global/client/transcript scope without claiming a live PASS.
