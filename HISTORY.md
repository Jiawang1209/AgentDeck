...
- **TDD evidence**: Gateway RED produced 11 failures for the missing candidate field and four-field diagnostics; durable session RED produced four failures for absent response/event facts and authority classification; landing RED produced 13 failures for missing validation, storage, and direct provenance. Expanded regressions cover local, JSON-object, prompt-only, native-schema, and ACP success provenance; exact constructor rejection; missing/extra/spoofed/wrong-type authority/schema/attempt/regeneration facts; zero domain writes; deep-copy behavior; legacy compatibility; and hash stability.
