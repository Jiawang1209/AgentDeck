...
- **TDD evidence**: the initial RED was the missing lease module; follow-up REDs covered audit action shape, lock-file zero-write, exact preview objects and timestamps, TTL conversion/offset overflow, forged backward/expired renewal and release, incoherent first-grant timestamps, hand-constructed/copied-capability/tampered privileged transitions, mutable audit payloads, malformed/duplicate outbox facts, restart-safe repeated expiry, malformed/duplicate-key/duplicate-identity journal records, non-expiry journal-scan avoidance, legacy event identity compatibility, non-ASCII secret diagnostics, and malformed persisted leases. The focused suite is 59 passed, the combined lease/conversation/protocol/daemon regression is 363 passed, and compileall plus `git diff --check` pass.
