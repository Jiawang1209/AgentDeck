...
- **Sanitized Leader planning diagnostics**: CLI-backed Leaders now classify timeout, nonzero exit, JSON parse, schema, cancellation, and oversized output through a fixed typed stage allowlist. Raw stdout/stderr, prompts, argv, executable paths, and exception text never enter the exception payload or durable conversation record. `LeaderGateway` preserves the typed stage; a failed or cancelled conversation turn immediately commits its terminal transition and a compact `conversation_turn_terminal.stage`, so clients and acceptance tooling need not wait for a Mission that cannot exist. Natural-language Mission planning now derives the requested Worker set once through `mission_intent` plus `select_mission_agents`, passes only that selected config to the Leader, and leaves preview authority on the same selection path; an unrelated third configured Worker is neither exposed to the planning prompt nor authorized by the preview.
