...
- **TDD evidence**: Gateway RED produced 11 failures for the missing candidate field and four-field diagnostics; durable session RED produced four failures for absent response/event facts and authority classification; landing RED produced 13 failures for missing validation, storage, and direct provenance. Spec review added 13 Gateway RED failures for lost post-generation attempt facts and explicit authority reaching executable probe/provider construction, while the stale strict fixtures produced two acceptance/Mission RED failures before exact provenance migration. Final review added one RED proving derived `None`/`None` authority from a one-Worker config still reached probe/factory; the paired valid legacy regression remained green. Expanded regressions cover local, JSON-object, prompt-only, native-schema, and ACP success provenance; cancel/serialization/oversize after validated local/native results; exact constructor rejection; missing/extra/spoofed/wrong-type authority/schema/attempt/regeneration facts; zero backend/domain effects on explicit or derived invalid authority; deep-copy behavior; legacy compatibility; and hash stability.
