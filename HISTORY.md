...
- **Verification**: the final focused authoritative-state/daemon CLI/Mission group passes 368 tests, the required daemon/conversation/contracts/agent-CLI/dashboard regression passes 1,850 tests, and the full suite passes 2,745 tests with one skip. `python -m compileall -q src tests`, `git diff --check`, and the no-temporary-daemon/ACP-worker process audit pass.
