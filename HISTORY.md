...
- **TDD evidence**: RED first exposed the missing retry-limit preview API, duplicate Mission attempt identities, dropped declared-test/acceptance hashes, four accepted prior-attempt authority drifts, an accepted existing-existing duplicate dispatch key, six accepted candidate-boundary failures (ID collision plus invalid/naive/backward clocks), and a split event timestamp. GREEN is 55 passed for the focused snapshot/attempt suite, 134 passed for the required Mission/conversation/workflow regression, 631 passed for Mission/contracts/dispatch/conversation-state, and 6 passed for targeted trace regression; compileall and `git diff --check` pass.
