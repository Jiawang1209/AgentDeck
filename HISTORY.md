...
- **TDD evidence**: initial RED failed collection because `agentdeck.daemon.recovery` did not exist. Follow-up REDs covered persisted Mission/attempt drift, forged validated/handoff facts, terminal and pending-confirmation precedence, illegal decision transitions, orphan/cross-Mission references, multiple active attempts, multi-Mission corruption, state drift between read and commit, forged direct persistence, confirmation-before-first-attempt recovery, unaudited reply evidence, controlled permission status drift, and protocol-transition CAS drift with an unchanged recovery outcome. GREEN is 296 passed for recovery/snapshot/scheduler/supervisor, 237 passed with one skip for the conversation/protocol/ACP/tmux/workflow expansion, and 2,493 passed with one skip for the full suite; compileall and `git diff --check` pass.
