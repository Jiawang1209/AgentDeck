...
- **Frozen retry authority**: retry allowance comes only from the frozen snapshot `retry_limit`; zero permits one total prepared attempt and one permits at most two. The lock-bound scan counts all Mission/step history, permits retry only after explicit `failed`, rejects active, completed, succeeded, cancelled, interrupted, ambiguous, exhausted, malformed, duplicate-id, and duplicate-key histories before writing, and incorporates deterministic attempt ordinal into each dispatch key.
- **TDD evidence**: RED first exposed the missing retry-limit preview API, duplicate Mission attempt identities, and dropped declared-test/acceptance hashes. GREEN is 44 passed for the focused snapshot/attempt suite, 123 passed for the required Mission/conversation/workflow regression, 631 passed for Mission/contracts/dispatch/conversation-state, and 6 passed for targeted trace regression; compileall and `git diff --check` pass.
