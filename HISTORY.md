...
- **Anchored event journal and mutation lock**: every `events.jsonl` read and append now opens the journal relative to the already verified project `state` directory descriptor with `O_NOFOLLOW`, requires a regular file, and revalidates the opened inode against the named path before and after I/O. Generic event reads/appends and daemon, conversation, and protocol outbox drains share this boundary; an unsafe or replaced journal cannot redirect I/O outside the project, and a failed drain leaves its durable outbox intact for retry. After `flock`, the authoritative protocol mutation lock now proves its held descriptor is still the same regular inode named by `protocol-mutation.lock` and revalidates the state-directory anchor before exposing mutation authority, so lock-path replacement fails closed instead of creating two independent lock domains.
