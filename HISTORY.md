...
- **TDD evidence**: the initial RED was the missing client/server modules; follow-up REDs covered explicit request encoding, stalled request and blocked write deadlines, silent clients, external log symlinks, stale leases, path-delegated socket replacement, duplicate concurrent spawn, slow-subscriber close storms, cross-restart log caps, handler shutdown, spawn-lock descriptor cleanup, multiplied connect-phase timeouts, and stale startup-loop remaining time. The real subprocess fixture acquires Task 3 ownership, installs the child-side bounded stdio, serves a verified endpoint, reconnects, and cleans up. A five-run concurrency/backpressure/shutdown loop exposed and then eliminated an unretrieved EOF task by gathering completed as well as cancelled connection tasks. The final IPC suite is 28 passed under `PYTHONASYNCIODEBUG=1` with warnings as errors and no pending-task, transport, callback, descriptor, or resource warning; the combined protocol/process/lease/IPC regression is 201 passed, and compileall plus `git diff --check` pass.
