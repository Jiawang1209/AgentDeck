...
- **Final Leader authority recovery closure**: explicit round/step quantities now pass through one conservative numeric-shape detector before any Leader call. Unsupported Chinese forms (`两百`-style and financial numerals), decimal/full-width/signed formatting, out-of-range values, huge tokens, and a later invalid quantity after an earlier valid phrase all fail with the fixed `mission step count invalid` boundary; standalone `两` remains the valid integer two, and ordinary ordinal/label prose still retains the compatible default. If an exact preview proof fails after durable state already terminalized the turn, the session reads the validated durable turn projection and returns a fixed `durable_state` fail-stop card instead of attempting an illegal `waiting_leader -> failed` transition, fabricating a preview, repeating domain writes, or leaking an exception. It appends only the compact `conversation_turn_recovery_blocked` audit when the remaining state permits a safe append. Exact preview recovery also requires each expected event identity/content to appear at most once in each durable channel; one identical journal record plus one pending outbox replay is valid, while any same-channel duplicate rejects recovery.
- **Final Leader recovery verification**: focused Mission/session coverage passes `64` tests, conversation/provider/acceptance coverage passes `213` tests, and the complete daemon suite passes `963` tests. `python -m compileall -q src tests` and `git diff --check` pass.
