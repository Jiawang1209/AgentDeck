...
- **Global authoritative state transaction**: every public `StateStore` entrypoint that transitively writes `state.json` now enters the same anchored `.agentdeck/protocol-mutation.lock` transaction before its first load and holds it through atomic replacement. Same-thread nesting is reentrant, internal write helpers reuse the outer transaction, and the former mission-only execution lock is removed. A static call-graph test requires the explicit 68-entry registry to equal all public transitive writers. A bounded process-local source-hash sidecar preserves plain-dict compatibility while rejecting stale public saves after migration or another process commit; every internal atomic save refreshes that source fact. The lock sidecar is created by normal project layout, so validation failures remain full-tree zero-write, while config-only reads remain zero-write and the first legitimate mutation safely creates a missing state directory.
...
- **Verification**: the final focused authoritative-state/daemon CLI/Mission group passes 356 tests, the required daemon/conversation/contracts/agent-CLI/dashboard regression passes 1,838 tests, and the full suite passes 2,733 tests with one skip. `python -m compileall -q src tests`, `git diff --check`, and the no-temporary-daemon/ACP-worker process audit pass.
