...
`agentdeck workbench` 是面向未来 GUI/TUI 的只读一屏快照。它会先校验 ProjectView，再组合 `project_view`、`leader_actions`、`runtime_card`、`ledger_card`、`recovery`、`continue_card`、`active_queue_source`，以及当前 recovery 指向的 `inbox_card`、`approval_card` 或 `leader_action`。`runtime_card` 会从 ProjectView 派生每个 agent 的 role、provider、workspace_mode、runtime status、pane_id、session_name、cwd，以及 `spawn_command`、`stop_command` 和 `inbox_command`，供 GUI 直接渲染可见 tmux runtime 控制面。`ledger_card` 会从 ProjectView 派生 messages/jobs/replies/inbox 摘要，并汇总去重后的 `trace_commands`，供 GUI 从一屏状态跳转到 `agentdeck trace --id <id>`。输出前会通过 `validate_workbench_contract()` 自校验；它不创建 plan、不记录 chat turn、不 ack、不 approve、不 dispatch、不 capture reply、不读取 pane 输出、不发送 tmux 输入。
