...
`agentdeck workbench` 的 `leader_card.controls[]` 也会进入 `agentdeck controls`，以 `scope=leader` 暴露 `chat`、`continue`、`review`、`actions`、`leader_status` 和完整 ProjectView `status`。其中 `leader_status` 指向窄版 `agentdeck leader status`，适合 GUI 顶栏或命令面板直接渲染 Leader/provider/recovery 状态入口；它只是 inspect control，不会调用 provider、读取 tmux 或写 state。
