...
AgentDeck 是一个正在搭建中的本地多智能体终端工作台。它的目标是让任意可通过 API 调用的 LLM 作为 Leader Agent，把任务分发给多个 Worker Agent，按需加载可审计 skill，并在 tmux 可见终端中执行、观察、审批、恢复和审计。DeepSeek 可以作为首个默认 provider，但不是架构绑定点。Skill 是北极星的一等生态接口：AgentDeck 应内置少量高价值工作流 skill，也允许外源 `SKILL.md` 经预览、hash、显式导入、显式加载和审计后进入项目，但不得静默安装、自动启用或绕过审批。控制模式会保持类似 Codex 的梯度：默认可 ask/inspect，由人类显式审批或授权后再进入更高自治度的执行。
...
agentdeck leader chat --message "查看所有计划"
agentdeck leader chat --message "浏览技能源"
...
`agentdeck skills list` 会发现内置 skill 和 `.agentdeck/skills/<name>/SKILL.md` 项目本地 skill，并返回 `import_command_template`、列表级 `controls[]` 以及每个 skill 的 show/load `controls[]`，方便 GUI 直接渲染 skill registry；`agentdeck skills catalog --source <dir>` 是 skill-marketplace lane 的第一片：只读浏览一个本地 skill 源目录（`<name>/SKILL.md`）这扇"橱窗"，每个 skill 带 provenance/hash、三态 `import_status`（`not_imported` / `imported_identical` / `imported_differs`，按 name+content_hash 与项目已导入 skill 比对）和显式 `import-preview` / `import` 命令，但只读浏览绝不安装——复制文件、追加事件、调用 provider、读取 tmux 一概不做，真正安装仍走既有的、预览门控且可审计的 `agentdeck skills import --path <SKILL.md>`；`.agentdeck/config.toml` 的 `[skills] allowed_sources` 是手工维护的可信 skill 源清单（默认空、会随 `_dump_config` round-trip 保留），`agentdeck skills sources` 只读列出这些配置源（`mode=skills_sources`、`source_count`、`sources[]` 每项 `{path, exists, catalog_command}` 和 inspect controls），而 `skills catalog` 顶层 `source_allowlisted` 标注被浏览的 `--source` 是否等于或位于某个配置源之下——catalog/sources 这层是**非强制**标注：任何目录仍可只读浏览、catalog 仍列全部 skill；强制只落在 `skills import`（见下）；自然语言 `agentdeck leader chat --message "浏览技能源"`（及 `查看技能源` / `技能源` / `技能市场` / `技能目录` / `skill sources` / `skill catalog`）会进入只读 `mode=skills_catalog`，嵌入无参的 `skills_catalog_card`（复用 workbench 同源卡片，概览配置的 `[skills] allowed_sources`），把下一步停在显式 `agentdeck skills sources`，只记录 chat turn 和审计事件，不调用 provider、不读取 tmux、不 import/load skill、不修改任何 state；`agentdeck skills import-preview --path /path/to/SKILL.md` 会只读解析外源 `SKILL.md`，返回 source、目标项目路径、hash、是否覆盖，以及 import / force_import / show_after_import controls，不复制文件、不追加事件、不 load、不调用 provider、不读取 tmux 或修改 runtime；自然语言 `agentdeck leader chat --message "预览导入 skill /path/to/SKILL.md"` 会复用同一张 `skill_import_preview_card`，只记录 chat turn 和审计事件，并把下一步停在显式 import/force import 命令。`agentdeck skills import --path /path/to/SKILL.md` 会把外部 `SKILL.md` 显式复制到项目 `.agentdeck/skills/<name>/SKILL.md` 并追加 `skill_imported` 审计事件，默认不覆盖同名 skill，必须加 `--force` 才能覆盖；import 还会对 `[skills] allowed_sources` 做 opt-in 强制——allowlist 为空时行为不变（向后兼容），allowlist 非空且 source 不在任一可信源下时会拒绝导入（不复制、不写事件、返回非 0），只有显式 `--allow-unlisted` 逃生阀可越过，两条路径都通过 `skill_imported.allowlisted` / `.allow_unlisted` 审计；`skills import-preview` 会只读回显 `source_allowlisted` / `enforcement_active` / `import_blocked` 让人在导入前看到这道门（预览本身不阻断、不导入）。`agentdeck skills show --name <name>` 只读返回 skill 内容、source、path、hash、required_tools、risk 和同源 controls；`agentdeck skills load-preview --name <name> --agent <id> --purpose <text>` 与自然语言 `agentdeck leader chat --message "预览加载 skill planning 给 planner 用于 decompose implementation work"` 会先返回只读 `skill_load_preview` / `skill_load_preview_card`，展示目标 agent、用途、skill summary、显式 load command 和 show/load controls，并附带只读依赖提示 `unmet_dependencies`（该 skill 声明的 `depends_on` 中尚未出现在已发现 skill 里的名字）和 `has_dependency_cycle`，仅供人在 load 前看到缺失依赖——它不阻断 preview、不 auto-load/auto-import 依赖，也不写入 `skill_loads[]`、不追加 `skill_loaded`、不调用 provider、不读取 tmux、不创建 plan/action/approval/message/job/inbox；`agentdeck skills load --name <name>` 才会把该 skill 的 path/source/hash/content snapshot、调用者和用途写入 `skill_loads[]`，并追加 `skill_loaded` 审计事件。`depends_on` 条目可选地约束依赖版本：纯 `name` 表示任意版本（行为不变）；`name@sha256:<hex>`（决策 B-ver）锁定内容 hash，要求当前依赖的 `content_hash` 与 pin 完全一致；`name@<spec>`（决策 semver，`<spec>` 不以 `sha256:` 开头）是 semver 范围，与依赖 `SKILL.md` frontmatter 声明的 `version: X.Y.Z`（默认 `0.0.0`）比对——支持 bare/`==` 精确、`>=` `>` `<=` `<`、caret `^X.Y.Z`、逗号 AND（`>=1.2,<2.0`），不支持或无法解析的范围一律 fail-safe 当硬 blocker。任何不满足（hash 不符或范围不满足）都记为 `version_mismatch`（`{name, expected, actual, reason}`），`resolve_skill_dependencies` / `skills deps` / `skills load-plan` 报告该 blocker 类别，`skills load --with-deps --confirm` 一律硬阻断拒绝、零写；纯 stdlib、本地、确定性、无网络。`agentdeck skills suggest --name <name> --summary <summary> --rationale <rationale> --source <source>` 会把可复用工作流沉淀为 pending suggestion，写入 `skill_suggestions[]` 并追加 `skill_suggested` 审计事件，但不创建 `SKILL.md`、不 import、不 load、不调用 provider、不读取 tmux；`agentdeck skills suggestions` 与自然语言 `agentdeck leader chat --message "查看 skill 建议"` 都只读列出 suggestion queue，并通过 `skill_suggestions_card` 暴露 count、pending_count、items 和 inspect controls，且自然语言响应会附带过滤到 `scope=skills` / `card=skill_suggestions_card` 的 `control_registry_card`，selection 指向 `agentdeck skills suggestions`，供 GUI 或人类决定后续是否手动创建/导入/load；`agentdeck skills draft-preview --suggestion-id <id>` 会把 pending suggestion 只读转换为拟写入的 `SKILL.md` 内容、hash、目标路径和显式 create control，不创建文件、不更新 suggestion 状态、不追加事件、不 import/load、不调用 provider 或读取 tmux；自然语言 `agentdeck leader chat --message "创建 skill 建议 sgs_xxx"` 会进入只读 `mode=skill_create_preview`，嵌入同源 `skill_create_preview_card` 并返回显式 `agentdeck skills create --suggestion-id <id> --confirm`，但不会创建文件或修改 suggestion 队列；`agentdeck skills create --suggestion-id <id> --confirm` 才会把同一份草稿写入项目 `.agentdeck/skills/<name>/SKILL.md`，标记 suggestion 为 `created` 并追加 `skill_created`，但不自动 load、不调用 provider、不读取 tmux 或修改 runtime。`agentdeck memory suggest --summary <summary> --rationale <rationale> --source <source>` 会把值得长期记住的项目事实或偏好写入 pending `memory_suggestions[]` 并追加 `memory_suggested` 审计事件，`agentdeck memory suggestions` 与自然语言 `agentdeck leader chat --message "查看 memory 建议"` 都只读列出队列，并在每条 pending item 上返回 `apply_preview` / `apply_memory` controls；自然语言响应会附带过滤到 `scope=memory` / `card=memory_suggestions_card` 的 `control_registry_card`，selection 指向 `agentdeck memory suggestions`，方便 GUI 审阅长期记忆候选但不写入 memory；`agentdeck memory apply-preview --suggestion-id <id>` 会只读展示目标 `.agentdeck/memory/*.md`、是否会创建文件、将追加的 Markdown 内容和显式 apply control，不写 memory 文件、不修改 suggestion 状态、不追加事件、不注入 Leader/Worker prompt、不调用 provider 或读取 tmux；`agentdeck memory apply --suggestion-id <id> --confirm` 才会把同一份 Markdown 追加到 memory 文件，更新 suggestion 为 `applied`，写入 `applied_at` / `applied_path`，并追加 `memory_applied` 审计事件，缺少 `--confirm` 或重复 apply 都会拒绝。已应用长期 memory 会以 compact 摘要进入 ProjectView `memory`、workbench `memory_context_card` 和自然语言 `agentdeck leader chat --message "查看长期记忆"` 的 `memory_context_card`，只暴露 scope/path/hash/行数/字节数/preview，不暴露全文、不注入 Leader/Worker prompt、不调用 provider；自然语言响应会附带过滤到 `scope=memory` / `card=memory_context_card` 的 `control_registry_card`，selection 指向 `agentdeck status`。`agentdeck learn review --plan-id <id>`（省略 `--plan-id` 时默认最新保存的 plan，无 plan 时报错）是 Hermes 式学习复盘的只读入口：它复用现有 Leader summary / plan status / reply / artifact 事实，生成 GUI-ready `skills suggest` 与 `memory suggest` 后续命令，但不写 `skill_suggestions[]`、不写 `memory_suggestions[]`、不调用 provider、不读取 tmux、不自动创建 skill 或 memory；自然语言 `agentdeck leader chat --message "学习复盘 pln_xxx"` 会嵌入同源 `learning_review_card`，附带过滤到 `scope=learning_review` 的 `control_registry_card`，并把 selection 指向 `suggest_skill` control，但仍不执行任何 suggestion 命令。Skill load 不调用 provider、不读取 tmux、不发送输入、不改变权限；它会进入 ProjectView 的 `skills` 摘要，并在 `agentdeck workbench` 与自然语言 `agentdeck leader chat --message "查看已加载技能"` 中作为 `skill_context_card` 暴露给 Leader/GUI 使用。`agentdeck leader plan`、自然语言 plan 和 `agentdeck run --task` 会把 compact loaded skill 摘要传入 API-backed / CLI-backed Leader prompt，并把同一份 compact `skill_context` 固化到 plan record、ProjectView `plans.items[]` 和 `agentdeck plan status`；这些位置不会传完整 `content_snapshot`，也不会把 skill 当成 dispatch 或执行授权。`agentdeck dispatch` 和 `agentdeck approval dispatch` 会把目标 agent 已显式加载的 skill `content_snapshot` 注入该 worker 的任务 prompt，让 Worker 能真正按 skill 工作；同时 message 会保存 compact `prompt_skill_context`，并通过 ProjectView `messages.items[]`、workbench `ledger_card.messages.items[]` 和 `trace` 暴露给 GUI/审计使用。
...
agentdeck controls --scope autonomous
...
`agentdeck workbench` 的 `review_gate_card` 会把 artifact、code reviewer 回复和 round reviewer 配置汇总成验收门：`code_review` 可复用现有 `reviewer` / `code_reviewer`，`round_review` 必须显式配置 `round_reviewer`。只有存在 artifact、code review reply 和 round review reply 时，gate 才能进入 `ready`；否则返回 `blocked`、具体 `reason`、每个阶段的 `blocker` 和 trace/inbox inspect controls。顶层 `controls[]` 还会给出 disabled 的 `assign_code_reviewer` / `assign_round_reviewer` 模板命令，供 GUI 渲染显式角色配置表单；它们需要人类填入 `agent_id` 和 `role_prompt` 后再运行。自然语言 `agentdeck leader chat --message "查看验收门"` 会复用同一张 `review_gate_card`，并附带过滤到 `scope=review_gate` / `card=review_gate_card` 的 `control_registry_card`，selection 指向 `agentdeck workbench` inspect control。它不会 release、merge、ack、dispatch 或推进下一轮，只让 GUI/TUI 把“为什么还不能进入下一轮”显示出来。

`agentdeck workbench` 还会暴露 `release_preview_card`：它只从 `review_gate_card` 派生，`can_release` 与验收门保持一致；当验收门 blocked 时，它复用同一个 `reason`，并把 `release_preview` / `next_round_preview` 控件保持为 disabled `explicit_user` 且 `command=null`，唯一 enabled control 是回到 `agentdeck workbench` 检查验收门。当验收门 ready 时，`release_command` / `next_command` 指向显式 `agentdeck release --confirm`，`release_preview` 控件变为 enabled `explicit_user`，`next_round_command` 暴露 disabled `agentdeck leader plan --task <goal>` 模板（blocker 为 `requires goal text`）。当前 code review / round review reply 对已经 release 过时，卡片进入 `status=released`：`already_released=true`、reason 为 `round already released`、release 命令收回为 `null`，只保留下一轮 plan 模板作为后续入口。`already_released` / `release_count` / `latest_release_id` 都派生自 ProjectView 顶层 `releases` 摘要（`agentdeck status` 同源暴露轮次历史，每条带指向 round review reply 的 `trace_command`）；渲染卡片本身永远不会执行 release。自然语言 `agentdeck leader chat --message "查看发布预览"` 会进入只读 `mode=release_preview`，嵌入同源 `release_preview_card`，附带过滤到 `scope=release_preview` / `card=release_preview_card` 的 `control_registry_card`，selection 指向 `inspect_review_gate` control。也就是说它只是给 GUI/TUI 预留“发布 / 下一轮”决策位，不会 release、merge、ack inbox、dispatch follow-up、推进 loop 或调用 provider。

`agentdeck release --confirm` 是验收门之后唯一的显式 release 落地命令：不带 `--confirm` 必须失败且不写 state；带 `--confirm` 时复用同一份 `review_gate_card` 事实，gate blocked 或本轮（同一对 code review / round review reply）已 release 过都会拒绝并追加 `round_release_rejected` 事件；gate ready 时把 release 记录写入 `releases[]` 并追加 `round_released` 审计事件。它只固化“人类确认本轮验收通过”这件事，不 merge、不 ack、不 dispatch、不创建 plan/approval、不调用 provider、不碰 tmux；下一轮仍从显式 `agentdeck leader plan --task <goal>` 开始。成功输出在打印前必须通过 `validate_release_contract()` 自校验；GUI/外部集成可通过只读 `agentdeck contract release` / `--example` 发现 release 响应字段、release 记录字段和 controls 字段（契约文档在 `docs/contracts/release-schema.md`，并已进入 `agentdeck contract list` 索引）。

`agentdeck workbench` 还会暴露 `role_topology_card`（北极星 Phase G6 角色拓扑）：它把 logical Leader 协调角色（`frontdesk`、`planner`、`orchestrator`）和配置的 worker 角色合成一个有序列表，让 GUI/TUI 一眼看出每个角色的 provider、生命周期、当前状态、阻塞点和只读的下一步入口。logical 角色从 `leader.coordination_roles[]` 派生，保持 `kind=logical_role`、`runtime_kind=logical_role`、`pane_backed=false`、`pane_id=null`、`agent_id=null`，`status` 由 ProjectView 事实推导（frontdesk 恒为 `ready`；planner 有计划时 `planning` 否则 `idle`；orchestrator 有 pending 审批时 `waiting_for_approval`（带 blocker `waiting for human approval`），否则有 pending Leader action 时 `coordinating`，否则已 release 过时 `released`，否则 `idle`），只有 orchestrator 会带 blocker，frontdesk/planner 的 blocker 恒为 `null`；每个 logical 角色的 inspect 控件指向自己的只读状态源（frontdesk→`agentdeck leader chat-history`、planner→`agentdeck plan list`、orchestrator→`agentdeck leader actions`）。worker 角色从同一份 `worker_lifecycle_card` 派生，`kind=worker`、`runtime_kind=worker_pane`，复用 worker `lifecycle_stage` 作为 `status`，inspect 控件指向 `agentdeck inbox --agent <id>`。当某个 worker 的 `agent_id` 是 `review_gate_card` 某一阶段（`code_review` 优先于 `round_review`）的 reviewer 时，拓扑会把该阶段状态叠加到这个角色上：`ready` → `status=reviewed`，`waiting_for_review` → `status=reviewing`（轮到它审查，无 blocker），其它情况（如 `waiting_for_artifacts`、`blocked`）→ `status=blocked` 并带上阶段 `blocker`；非 reviewer 角色保留基础 lifecycle 状态且 blocker 为 `null`。所有控件都进入 `agentdeck controls` 的 `scope=role_topology`；渲染卡片本身不 spawn、不 dispatch、不 capture、不 ack、不 release、不写 state。自然语言 `agentdeck leader chat --message "查看角色拓扑"` 会进入只读 `mode=role_topology`，嵌入同一张 `role_topology_card`，附带过滤到 `scope=role_topology` / `card=role_topology_card` 的 `control_registry_card`，selection 指向 card 级 `agentdeck workbench` inspect control；它只记录 chat turn 和审计事件，不调用 provider、不创建 plan/action/approval/message/job/inbox、不读写 tmux。
...
`agentdeck workbench` 还会在最新 plan 达到 `next_action=summarize` 时嵌入 `learning_review_card`（否则为 `null`）：它复用 `agentdeck learn review --plan-id <id>` 的响应形状并通过 `validate_learning_review_contract()` 守门，把 Hermes 式学习复盘直接带进一屏工作台，`summary` / `suggest_skill` / `suggest_memory` 控件进入 `agentdeck controls` 的 `scope=learning_review`。渲染工作台本身只读——不写 `skill_suggestions[]` / `memory_suggestions[]`、不调用 provider、不创建或加载 skill；真正入队仍必须人类显式运行 `agentdeck skills suggest ... --source learn-review` / `agentdeck memory suggest ... --source learn-review`。
...
`agentdeck status` 是当前面向 CLI、自然语言入口和未来 GUI 的统一只读 ProjectView。它会返回项目配置、Leader、agents runtime binding、state_path，以及 plans、approvals、messages、jobs、replies、artifacts、chat_turns、leader_errors、leader_actions、skills、memory、inbox、recovery 的轻量摘要。顶层 `leader.leader_backend` 会暴露当前配置 provider/model 的 normalized logical Leader identity，供 GUI 在没有 plan 时也能识别 fake、API-backed 或 CLI-backed Leader 来源；它不是 tmux pane 绑定、provider readiness 证明或执行授权。
...
`agentdeck workbench` 是面向未来 GUI/TUI 的只读一屏快照。它会先校验 ProjectView，再组合 `project_view`、`leader_actions`、`leader_card`、`provider_health`、`runtime_card`、`agent_ready_card`、`terminal_session_card`、`role_card`、`ledger_card`、`lineage_card`、`queue_card`、`operator_card`、`audit_card`、`artifacts_card`、`skill_context_card`、`skill_suggestions_card`、`memory_suggestions_card`、`leader_summary_card`、`learning_review_card`、`contracts_card`、`control_mode_card`、`recovery`、`continue_card`、`active_queue_source`、`run_progress_card`，以及当前 recovery 指向的 `inbox_card`、固定 Leader mailbox 投影 `leader_inbox_card`、`approval_card` 或 `leader_action`。`leader_card.leader_backend` 会暴露当前配置 provider/model 的 normalized logical Leader identity，让 GUI 一进入工作台即可区分 fake、API-backed 或 CLI-backed Leader 来源；该字段不是 tmux pane 绑定、readiness 证明或执行授权。`ledger_card.messages.items[].prompt_skill_context` 会复用 ProjectView message 摘要中的 compact worker skill provenance，帮助 GUI 在通信账本中直接显示“这次任务给 worker 注入了哪些 skill”。`skill_suggestions_card` 会把 pending skill 建议队列直接带到一屏工作台，并在 control registry 里暴露 `agentdeck skills suggestions` inspect control，方便 GUI 审阅 Hermes 式 skill 沉淀结果但不自动创建、导入或加载 skill。`memory_suggestions_card` 会把 pending memory 建议队列直接带到一屏工作台，暴露 `agentdeck memory suggestions` inspect control、只读 `agentdeck memory apply-preview --suggestion-id <id>` 控件和显式 `agentdeck memory apply --suggestion-id <id> --confirm` 控件，方便 GUI 审阅长期记忆候选并把最终写入交给人类显式命令；渲染 workbench 本身不创建或修改 `.agentdeck/memory/*.md`，也不注入 prompt。`control_mode_card` 把类似 Codex 的授权梯度显式化：`ask` 表示只计划、观察和建议命令；`approve` 表示沿用已有的人类审批与 safe apply；`autonomous` 现在是已实现的有界放权模式：available_mode 已 enabled，但其 `set_mode` 控件是 disabled 模板（blocker `requires --allow-agent and --max-approvals`），提醒 GUI 必须填好 allowlist 和预算才能显式启用。`provider_health` 会从 Leader provider 和本地环境变量派生 `provider_backend`、`provider_transport`、同源 `leader_backend`、supported、ready、missing_env、detail、doctor_command、doctor_contract、setup_commands 和 `controls[]`；这些 `controls[]` 是显式 provider 控制入口，覆盖 fake、DeepSeek、OpenAI-compatible、Codex CLI 和 Claude CLI，每个 provider 同时提供 `kind=set_provider` 普通切换、`kind=guarded_set_provider` 预检切换和 `kind=setup_provider` 安装/登录/placeholder export 修复命令，guarded 命令带 `--require-ready`；当前 provider switch disabled 并显示 `already current provider`，其他 provider 控件需要人类显式运行，不暴露密钥值、不调用 provider。其余 runtime、queue、ledger、artifact、skill suggestion、memory suggestion、contract 和 watch 行为仍保持只读：不创建 plan、不记录 chat turn、不 ack、不 approve、不 dispatch、不 capture reply、不读取 pane 输出、不发送 tmux 输入、不创建或加载 skill、不直接写长期 memory。

`agentdeck dashboard` 是一个只读 TUI 参考客户端：它复用 `agentdeck workbench` 的同一份契约 payload（同样先过 `validate_workbench_contract()`），只是把 JSON 渲染成人类可读的文本面板（header / recovery / role topology / review gate / queue），每个角色和 recovery/queue 的下一步命令都原样来自契约，不解析私有 state、不发明命令。它的价值是证明只读 workbench 契约足以驱动 GUI/TUI——渲染逻辑（`src/agentdeck/dashboard.py::render_workbench_dashboard`）是纯函数，只消费 payload dict，不写 state、不调用 provider、不 spawn/dispatch。`agentdeck dashboard --watch [--interval <秒>] [--iterations <n>]` 会按间隔重新渲染文本面板（对标 `agentdeck workbench --watch`），仍然只读。`agentdeck tui` 是同一份只读视图的 curses 交互版：可滚动的 overview + 可浏览的命令面板（从 `control_registry[]` 派生），`tab`/`p` 切换模式、方向键滚动/选择、`r` 刷新、`q` 退出；palette 每行显示控件的 scope/kind/label 和 `[x]`/`[ ]` enabled 标记，页脚显示选中控件的 safety、enabled/blocker 和要运行的确切命令（`run: <command>`）。它只是查看器——绝不执行命令、写 state、调用 provider 或碰 tmux;导航状态（`TuiModel`）和屏幕布局（`render_frame`）是纯函数、有单测,curses I/O 只是薄壳;非交互终端下 `agentdeck tui` 会优雅拒绝并指向 `agentdeck dashboard`。参考客户端说明和样例输出见 [docs/walkthroughs/tui-reference-client.md](/Users/liuyue/Desktop/Github_repos/multi-agent-explore/docs/walkthroughs/tui-reference-client.md)。
...
当人类输入 `agentdeck leader chat --message "帮助"`、`"help"`、`"/help"`、`"你能做什么"`、`"有哪些能力"`、`"命令面板"`、`"commands"` 或 `"capabilities"` 时，chat 会进入只读 `mode=help`：它返回 `capability_card`，列出当前自然语言入口支持的 plan、review、apply_action、continue、leader_status、workbench、runtime、role、ledger、audit、artifacts、queue、approval、inbox、policy、provider_switch、skill_context 和 setup 能力，以及每项能力的示例说法、推荐命令、safety、是否需要显式用户动作、对应卡片和 `controls[]`；其中 `leader_status.example_messages` 会包含 `查看 Leader 状态`、`刷新 Leader 状态`、`Leader 概览`、`leader status`、`leader refresh` 和 `leader overview`，`skill_context.example_messages` 会包含 `查看已加载技能` 和 `skill context`，让自然语言壳能主动展示同一张只读状态卡和已加载 skill 卡的中英文入口。同时返回 `control_registry_card`，从同一次 workbench snapshot 派生出 leader/provider/runtime/operator control registry，供 GUI 或自然语言壳直接渲染命令面板，且该 card 的 `default_command` 指向 `agentdeck controls`，并带有同一套 `filters` / `groups[]` 投影字段。自然语言也可以表达只读过滤和搜索，例如 `agentdeck leader chat --message "命令面板 runtime enabled only"` 会返回 runtime scope 的 enabled controls，`agentdeck leader chat --message "命令面板 搜索 terminal"` 会返回 query 匹配的 controls，但仍不执行任何 control。`plan` 使用 `safety=plan_only`，其 control 指向显式 `agentdeck leader plan --task <goal>`；`review` 和 `apply_action` 使用 `safety=safe_apply`，其中 review control 指向 `agentdeck leader review --plan-id <plan_id>`，apply_action control 指向 `agentdeck leader apply-action --action-id <action_id>`；`leader_status` 使用 `safety=inspect`，control 指向只读 `agentdeck leader status` 并声明 `leader_status_card`；`skill_context` 使用 `safety=inspect`，control 指向 `agentdeck leader chat --message "查看已加载技能"` 并声明 `skill_context_card`；`policy` 使用 `safety=explicit_user`，control 指向 `agentdeck policy set-mode --mode <mode>`；`provider_switch` 使用 `safety=explicit_user`，control 指向 `agentdeck leader set-provider --provider <provider> --model <model>`；只读视图能力使用 `safety=inspect`；模板 control 只能使用 `<goal>`、`<plan_id>`、`<action_id>`、`<agent_id>`、`<mode>`、`<provider>` 或 `<model>`，必须 disabled，并返回与占位符匹配的 blocker 说明缺少的具体输入。该模式建议 `agentdeck workbench`，只记录 chat turn，不调用 Leader provider，不创建 plan/action/approval/message/job/inbox，不读取 tmux pane，不发送 tmux 输入，适合作为未来 GUI 命令面板和自然语言壳的能力发现入口；能力项、controls 和 registry items 只是命令发现，不代表自动执行许可。

当人类输入 `agentdeck leader chat --message "切换到审批模式"`、`"回到 ask 模式"` 或包含 `autonomous` / `"完全放权"` 的控制模式意图时，chat 会进入 `mode=policy`：它嵌入与 workbench 同源的 `control_mode_card`，附带过滤到 `scope=policy` / `card=control_mode_card` 的 `control_registry_card`，并把 `next_command` 设置为显式的 `agentdeck policy set-mode --mode ask|approve|autonomous`。对应 `intent_card.controls[]` next label 会使用 `Switch to ask mode`、`Switch to approval mode` 或 `Request autonomous mode`，让 GUI 可以清楚渲染 ask/审批/放权梯度。registry selection 会指向目标 mode 的 `kind=set_mode` control：ask/approve 可用时 `selection.next_command` 等于顶层 `next_command`，autonomous 意图会把顶层 `next_command` 设为显式模板 `agentdeck policy set-mode --mode autonomous --confirm --allow-agent <id> --max-approvals <N>`，对应 `set_mode` control 是 disabled 模板（blocker `requires --allow-agent and --max-approvals`），`selection.next_command=null`。该模式只记录 chat turn，不修改 `.agentdeck/config.toml`，不创建 plan/action/approval/message/job/inbox，不调用 provider，不发送 tmux 输入；真正启用 autonomous 仍必须由人类填好 allowlist 和预算后显式运行该命令。

有界自主模式（autonomous）现已实现：`agentdeck policy set-mode --mode autonomous --confirm --allow-agent <id> --max-approvals <N>` 会校验 `--confirm`、非空 allowlist、`--max-approvals >= 1` 和 agent 已在配置中，通过后把 `leader.approval_mode` 设为 `autonomous` 并把 allowlist 和预算写入 `.agentdeck/config.toml` 的 `[autonomous]`，追加 `policy_mode_updated` 审计事件；缺 `--confirm`、缺 allowlist、预算非法或 agent 未知时会拒绝、保持配置不变并追加 `policy_mode_rejected`。启用后 `agentdeck approval auto --confirm` 是唯一自主执行入口：它只在 autonomous 模式下自动批准 allowlist 内、预算内的 pending approvals，并把它们派发给已有 running pane 的 agent（不 force-spawn，不启动新 pane），在 dispatch 处停下；不在 allowlist 或超预算的 approvals 会被跳过并附原因，目标 agent 没有 running pane 的会记为 blocked。每个自动批准（`approval_decided` with `source=autonomous`）、派发和一次 `approval_auto_completed` 都会写审计账本，可用 `agentdeck history` 复核（渲染为 `Approval auto-approved` 和 `Auto-approve run`）。

`agentdeck run-loop --plan-id <id> --confirm` 是只读 `agentdeck loop` 的 **write** 对应命令，也是有界自主模式的执行引擎。它在预授权的 autonomous 策略内把单个 plan 向前推进一步：先自动批准该 plan 内 allowlist、预算内的 pending approvals，再把 approved-and-ready 的审批项派发给已有 running pane 的 agent，然后复用 `leader review` 诊断当前卡在哪个人类 gate，并在那里停下、交回一条显式 `next_command`。它要求 `--confirm` **且** `leader.approval_mode == "autonomous"`，缺任一都会拒绝且不写任何 state；它绝不 force-spawn（目标 agent 没有 running pane 的审批记为 `blocked`，交回 `agentdeck agent spawn --agent <id>`），也绝不 capture reply 或推断 worker 完成（派发后等待回复时停在 `waiting_for_reply`，交回显式 `agentdeck capture-reply --agent <id> --message-id <id>`）。一次调用只执行一轮 auto-approve+dispatch wave，然后停在 gate，由人类清除 gate 后再运行。`stopped_reason` 取值为 `error`/`blocked`/`needs_human_approval`/`waiting_for_reply`/`complete`/`idle`。每次 wave 的 `approval_decided`（source=autonomous）、`approval_dispatched` 和一次 `run_loop_advanced` 汇总事件都会写审计账本，可用 `agentdeck history` 复核（渲染为 `Run-loop advanced`）。契约见 `docs/contracts/run-loop-schema.md`，可通过 `agentdeck contract run-loop`（`--example`）发现。

`agentdeck run-loop --all --confirm` 是**并行调度器**——把单计划 wave 扩展成对所有 active 计划跑一轮 **round-robin**。它按创建顺序遍历未完成的计划，每个复用同一套 run-loop wave（自动批准 + 派发），但共用一份 `max_approvals` 预算（整波总批准数 ≤ 预算），并在 agent 冲突时**跳过**——某 agent 只要有"已派发未回收"的任务，本波就不再派它，命中的审批记入该计划的 `skipped_contention`（仍保持 approved）；跑完一轮就停。它和 `run-loop --plan-id` 同样要求 `--confirm` + autonomous 模式（且二者互斥），安全边界完全一致（不 force-spawn、不自动回收、全程审计，汇总事件 `run_loop_all_advanced` 在 `agentdeck history` 渲染为 `Parallel wave`）。单计划 `run-loop --plan-id` 行为不变。契约见 `docs/contracts/run-loop-all-schema.md`，可通过 `agentdeck contract run-loop-all`（`--example`）发现。

这两条自主命令现在也以只读命令面板 control 的形式出现在 `agentdeck workbench` 的 `control_registry[]` 和 `agentdeck controls --scope autonomous` 下（派生自 `control_mode_card.autonomous_actions[]`）：`kind=approval_auto`（`agentdeck approval auto --confirm`，`safety=delegated`，只有 `approval_mode=autonomous` 时 enabled，否则 disabled 且 blocker 为 `autonomous mode is not enabled`）和 disabled 的 `kind=run_loop` 模板（`agentdeck run-loop --plan-id <id> --confirm`，blocker `requires --plan-id`）。渲染这些 control 只是让 GUI/TUI 可发现，不是执行授权——两条命令仍必须由人类显式带 `--confirm` 运行（`run-loop` 还需 autonomous 模式）。

自然语言也能发现 run-loop：`agentdeck leader chat --message "推进计划 pln_xxx"`（以及 `往前推`、`驱动计划`、`run-loop` 等变体）会进入只读 `mode=run_loop_preview`，嵌入 `run_loop_preview_card` 并把显式 `agentdeck run-loop --plan-id <id> --confirm` 交回为顶层 `next_command`，同时附带过滤到 `scope=autonomous` 的 `control_registry_card`（selection 指向 disabled `run_loop` 模板 control）。这个入口是只读建议：它必须带 plan id（不猜测），autonomous 模式关闭时 intent next control 会 disabled 并给出 `autonomous mode is not enabled` blocker 和 `enable_command`；chat 本身只记录 chat turn 和审计事件，绝不调用 provider、读写 tmux、auto-approve、dispatch 或修改 approval/runtime/plan state——执行 run-loop 仍由人类显式运行。
