...
`agentdeck workbench` 的 `agent_ready_card.controls[]` 会进入 `agentdeck controls`，以 `scope=agent_ready` 暴露 `inspect`、`spawn_ready`、`refresh_runtime` 和必要时的 `dispatch_ready`；自然语言 `agentdeck leader chat --message "启动所有 agent"` 和 `"启动 planner"` 都会返回 `startup_preview_card`，把顶层显式启动命令展开成待启动 agent 清单、单 agent spawn 命令、terminal 命令、blocker 和 per-agent controls，供 GUI 在执行前确认；单 agent spawn 响应还会返回过滤到 `startup_preview_card` 的 `control_registry_card`，用 `selection.selected_control` 指向顶层 `next_command` 对应的 spawn control；`agentdeck workbench` 的 `terminal_session_card.controls[]` 会进入 `agentdeck controls`，以 `scope=terminal_session` 暴露 `attach_session`、`open_controls` 和 `refresh_runtime`；每个 `terminal_session_card.terminals[]` item 自带的 GUI-ready `controls[]` 也会进入同一 scope，以 `kind=select_pane` / `agent_id=<agent>` 渲染单 pane 选择按钮。GUI 可以把它渲染成项目级终端工具条或命令面板分区：attach/open/select-pane 是 inspect，refresh 是 explicit_runtime；这些 item 只提供可见命令，不会自动 attach tmux、select pane、refresh runtime 或写 state。

自然语言 `agentdeck leader chat --message "刷新 runtime"`、`"发送给 planner：继续"` 和 `"停止 planner"` 现在都会返回 `runtime_action_card`，把项目级 refresh 或目标 agent、role、runtime status、pane_id、待发送文本或停止动作、精确 `agentdeck agent refresh` / `agentdeck agent send --agent <id> --text <text>` / `agentdeck agent stop --agent <id>` 命令和 inspect/refresh/send/stop controls 结构化出来；同一响应会附带 `runtime_card`、`terminal_session_card` 和过滤到 `runtime_action_card` 的 `control_registry_card`，让 GUI 可以渲染刷新/发送/停止前确认按钮和目标 runtime 上下文。它仍只建议显式命令，不自动 refresh runtime、不发送 tmux 输入、不停止 pane、不创建 plan/action/approval/message/job/inbox，也不修改 runtime state。
