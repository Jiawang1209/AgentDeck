...
当你还没有配置 CLI-backed Leader 时，可以直接对自然语言入口说 `agentdeck leader chat --message "配置 Codex CLI Leader"`、`"登录 Codex CLI"` 或 `"安装 Claude CLI"`。这类请求会进入只读 `mode=setup`，返回当前 `provider_health`、过滤到 provider scope 的 `control_registry_card`，以及同源目标 `provider_switch_card`；`next_command` 仍是 allowlist 中第一条 setup 命令，例如 `codex login` 或 `claude auth`，而 `provider_switch_card.command` 给出配置完成后由人类显式运行的 `agentdeck leader set-provider --provider ...`。`leader_explanation.action_kind=provider_setup`，`intent_card.secondary_embedded_cards=["provider_switch_card","control_registry_card"]`。它只建议命令，不修改 `.agentdeck/config.toml`，不调用当前或目标 provider，不创建 plan/action/approval/message/job/inbox，也不发送 tmux 输入。
