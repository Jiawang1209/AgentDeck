...
`agentdeck workbench` 还会暴露 `release_preview_card`：它只从 `review_gate_card` 派生，`can_release` 与验收门保持一致；当验收门 blocked 时，它复用同一个 `reason`，并把 `release_preview` / `next_round_preview` 控件保持为 disabled `explicit_user` 且 `command=null`。当前唯一 enabled control 是回到 `agentdeck workbench` 检查验收门。自然语言 `agentdeck leader chat --message "查看发布预览"` 会进入只读 `mode=release_preview`，嵌入同源 `release_preview_card`，附带过滤到 `scope=release_preview` / `card=release_preview_card` 的 `control_registry_card`，selection 指向 `inspect_review_gate` control。也就是说它只是给 GUI/TUI 预留“发布 / 下一轮”决策位，不会 release、merge、ack inbox、dispatch follow-up、推进 loop 或调用 provider。
