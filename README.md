...
如果真实 provider 返回非法 JSON、缺少顶层 `goal` / `summary` / `steps`、缺少非空 steps、某个 step 缺少 `step` / `agent_id` / `role` / `task` / `risk` / `requires_approval`，或某个 step 没有 `requires_approval: true`，CLI 会明确失败并把错误写入 `.agentdeck/state/state.json` 的 `leader_errors[]`。失败不会创建 plan、approval、message、job 或 inbox。
