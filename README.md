...
`agentdeck workbench` 的 `review_gate_card` 会把 artifact、code reviewer 回复和 round reviewer 配置汇总成验收门：`code_review` 可复用现有 `reviewer` / `code_reviewer`，`round_review` 必须显式配置 `round_reviewer`。只有存在 artifact、code review reply 和 round review reply 时，gate 才能进入 `ready`；否则返回 `blocked`、具体 `reason`、每个阶段的 `blocker` 和 trace/inbox inspect controls。自然语言 `agentdeck leader chat --message "查看验收门"` 会复用同一张 `review_gate_card`，并附带过滤到 `scope=review_gate` / `card=review_gate_card` 的 `control_registry_card`，selection 指向 `agentdeck workbench` inspect control。它不会 release、merge、ack、dispatch 或推进下一轮，只让 GUI/TUI 把“为什么还不能进入下一轮”显示出来。
