...
`validate_workbench_contract()` 会拒绝 `provider_health.controls[]` 里非 `safety=explicit_user` 的 `kind=set_provider` 控件，也会拒绝缺少 blocker 的 disabled provider control，确保 GUI 里的 Leader provider 切换始终是人类显式动作。
