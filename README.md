...
自然语言 `agentdeck leader chat --message "发送给 planner：继续"` 和 `"停止 planner"` 现在都会返回 `runtime_action_card`，把目标 agent、role、runtime status、pane_id、待发送文本或停止动作、精确 `agentdeck agent send --agent <id> --text <text>` / `agentdeck agent stop --agent <id>` 命令和 inspect/send/stop controls 结构化出来；同一响应会附带 `runtime_card`、`terminal_session_card` 和过滤到 `runtime_action_card` 的 `control_registry_card`，让 GUI 可以渲染发送/停止前确认按钮和目标 pane 上下文。它仍只建议显式命令，不自动发送 tmux 输入、不停止 pane、不创建 plan/action/approval/message/job/inbox，也不修改 runtime state。
...
发送输入和停止 agent 意图会额外把 `runtime_action_card` 作为 `intent_card.embedded_card`，并在 `secondary_embedded_cards` 中列出 `runtime_card`、`terminal_session_card` 和 `control_registry_card`。`runtime_action_card.controls[]` 的 `kind=send` / `kind=stop` 必须与顶层 `next_command` 完全一致，`control_registry_card.selection.selected_control` 也会指向同一个 send/stop control；GUI 可以据此展示“准备操作哪个 pane、发送什么文本或停止哪个 agent、复制/运行哪条命令”，但不能把该卡片当成自动发送或自动停止授权。
