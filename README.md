...
如果真实 provider 返回非法 JSON、缺少顶层 `goal` / `summary` / `steps`、`goal` / `summary` 不是非空字符串、缺少非空 steps、某个 step 缺少 `step` / `agent_id` / `role` / `task` / `risk` / `requires_approval`，`agent_id` / `role` / `task` / `risk` 不是非空字符串，`agent_id` 不在当前配置的 worker agent 列表中，或某个 step 没有 `requires_approval: true`，CLI 会明确失败并把错误写入 `.agentdeck/state/state.json` 的 `leader_errors[]`。失败不会创建 plan、approval、message、job 或 inbox。
...
真实 provider 仍然只生成 plan 或 chat turn，不会自动创建 approval 或派发任务。Leader planning prompt 会把每个 worker 的 `agent_id`、`role`、`role_prompt`、provider 和 workspace mode 一起交给 provider，让 DeepSeek、OpenAI-compatible、Codex CLI 或 Claude CLI 都能按角色职责拆分任务。`codex-cli` / `claude-cli` 是 `agent_id=leader` 这个逻辑 Leader 的本地推理后端，不会复用 `planner`、`coder` 或 `reviewer` 的 worker pane，也不会让 Leader 自动拥有一个 tmux pane；CLI prompt 会明确声明它只是本地 subprocess reasoning backend，不能声称自己占用或复用任何 worker tmux pane。`agentdeck leader plan/chat --model <model>` 或 `.agentdeck/config.toml` 中的 `[leader].model` 会进入真实 backend：API-backed provider 使用请求体 `model`，CLI-backed Leader 使用本地 `codex` / `claude` 的 `--model` 参数；CLI-backed Leader 可以解析纯 JSON stdout，或解析 Markdown fenced `json` block 中的 JSON plan；解析后仍必须通过同一 provider plan schema validator，确保 plan 有可展示的非空 `goal` / `summary`、每个 step 都具备非空且指向已配置 worker agent 的 agent/role/task/risk 调度字段，并停在人类审批门前。
