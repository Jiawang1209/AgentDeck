...
每次 dispatch 会写入 `.agentdeck/state/state.json` 的 `messages`、`attempts`、`jobs` 和目标 agent 的 `inbox`，并追加 `task_dispatched` 事件。若目标 agent 有 loaded skills，dispatch prompt 会包含这些 skill 的 full content snapshot；message 同时保存 compact `prompt_skill_context`，不把 full snapshot 再复制到摘要字段。`dispatch` 和 `approval dispatch` 的 JSON 输出会包含 `trace_command`，指向同一条 message lineage；`approval dispatch` 成功响应还会嵌入目标 agent 的 `inbox_card`，复用 `agentdeck inbox --agent <id>` 队列形状，让 GUI 或自然语言壳立刻看到 worker mailbox head、trace 和 ack 入口。可以查看某个 agent 的 inbox：
